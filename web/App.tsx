...
export default class App extends Component<void, IState> {
  ...
  async componentDidMount() {
    try {
      ...
      const widgetReady = new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(
          () =>
            reject(
              new Error(
                "Communication timed out to your Matrix client. Your browser may be blocking access.",
              ),
            ),
          5000,
        );
      ...
      widgetApi.on(
        `action:${WA.WidgetApiToWidgetAction.NotifyCapabilities}`,
        (ev) => {
      ...
      const widgetApiUrl = new URL(
        `${window.location.origin}${window.location.pathname.replace("/widgetapi/v1/static", "")}`,
      );
      const bridgeApi = await BridgeAPI.getBridgeAPI(
        widgetApiUrl.toString(),
        widgetApi,
      );
      const { userId } = await bridgeApi.verify();
      const roomState =
        widgetKind === "admin" ? await bridgeApi.state() : undefined;
      ...
      this.setState({
        ...
        embedType:
          embedType === EmbedType.IntegrationManager
            ? EmbedType.IntegrationManager
            : EmbedType.Default,
        kind: widgetKind,
        busy: false,
        widgetApi,
      ...
      if (ex instanceof BridgeAPIError) {
        if (ex.errcode === "M_AS_BAD_OPENID") {
          error =
            "Could not contact your homeserver. Your instance may be misconfigured.";
        }
      }
      this.setState({
        error,
  ...
  render() {
    const style = {
      padding:
        "embedType" in this.state &&
        this.state.embedType === EmbedType.IntegrationManager
          ? "0"
          : "16px",
    };
    if (this.state.error) {
      return (
        <div style={style}>
          <Alert type="critical" title="An error occured">
            {this.state.error}
          </Alert>
        </div>
      );
