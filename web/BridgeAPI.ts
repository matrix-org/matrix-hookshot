...
export class BridgeAPI {
    ...
    if (res.status !== 200) {
      if (res.headers.get("Content-Type")?.includes("application/json")) {
        ...
        throw new BridgeAPIError(
          resultBody?.error || "Request failed",
          resultBody,
        );
      } else {
  ...
  async getEnabledConfigSections(): Promise<{
  ...
  async getConnectionsForRoom(
    roomId: string,
  ): Promise<GetConnectionsResponseItem[]> {
    return this.request(
      "GET",
      `/widgetapi/v1/${encodeURIComponent(roomId)}/connections`,
    );
  }

  async getConnectionsForService<T extends GetConnectionsResponseItem>(
    roomId: string,
    service: string,
  ): Promise<GetConnectionsForServiceResponse<T>> {
    return this.request(
      "GET",
      `/widgetapi/v1/${encodeURIComponent(roomId)}/connections/${encodeURIComponent(service)}`,
    );
  }
  ...
  async createConnection(
    ...
    return this.request(
      "POST",
      `/widgetapi/v1/${encodeURIComponent(roomId)}/connections/${encodeURIComponent(type)}`,
      config,
    );
  }
  ...
  async updateConnection(
    ...
    return this.request(
      "PUT",
      `/widgetapi/v1/${encodeURIComponent(roomId)}/connections/${encodeURIComponent(connectionId)}`,
      config,
    );
  }
  ...
  getConnectionTargets<R>(
    type: string,
    filters?: Record<never, never> | Record<string, string>,
    abortController?: AbortController,
  ): Promise<R[]> {
    const searchParams =
      filters && !!Object.keys(filters).length && new URLSearchParams(filters);
    return this.request(
      "GET",
      `/widgetapi/v1/targets/${encodeURIComponent(type)}${searchParams ? `?${searchParams}` : ""}`,
      undefined,
      { abortController },
    );
  }
  ...
  async getAuthPoll(
    service: string,
    state: string,
  ): Promise<GetAuthPollResponse> {
    return this.request(
      "GET",
      `/widgetapi/v1/service/${service}/auth/${state}`,
    );
  }
