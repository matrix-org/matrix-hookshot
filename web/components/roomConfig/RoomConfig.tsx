...
export interface ConnectionConfigurationProps<
  SConfig,
  ConnectionType extends GetConnectionsResponseItem,
  ConnectionState extends IConnectionState,
> {
...
> {
  ...
  connectionConfigComponent: FunctionComponent<
    ConnectionConfigurationProps<SConfig, ConnectionType, ConnectionState>
  >;
}

const MAX_CONNECTION_FETCH_ATTEMPTS = 10;

export const RoomConfig = function <
  SConfig,
  ConnectionType extends GetConnectionsResponseItem,
  ConnectionState extends IConnectionState,
>(props: IRoomConfigProps<SConfig, ConnectionType, ConnectionState>) {
  const {
    roomId,
    type,
  ...
  const [newConnectionKey, incrementConnectionKey] = useReducer<
    number,
    undefined
  >((n) => n + 1, 0);
  ...
  const handleSaveOnCreation = useCallback(
    (config: ConnectionState) => {
      isUpdatingConnection(true);
      api
        .createConnection(roomId, connectionEventType, config)
        .then((result) => {
          // Force reload
        ...
        .catch((ex) => {
          ...
          setError({
            ...
            message:
              ex instanceof BridgeAPIError ? ex.message : "Unknown error",
          });
        })
        .finally(() => {
      ...
      <main>
        {showHeader && (
          <header className={style.header}>
            <img
              alt=""
              className={darkHeaderImg ? style.invert : undefined}
              src={headerImg}
            />
            <h1>{text.header}</h1>
          </header>
        ...
        {!canSendMessages && canEditRoom && (
          <Alert type="info" title={"Misconfigured permissions"}>
            This room does not permit the bot to send messages. Please go to the
            room settings in your client and adjust permissions.
          </Alert>
        )}
        {canEditRoom && (
          <section>
        ...
        {!!connections?.length && (
          <section>
            ...
            {serviceConfig &&
              connections?.map((c) => (
                <ListItem key={c.id} text={listItemName(c)}>
                  <ConnectionConfigComponent
                    serviceConfig={serviceConfig}
                    ...
                    onSave={(config) => {
                      isUpdatingConnection(true);
                      api
                        .updateConnection(roomId, c.id, config)
                        .then(() => {
                          c.config = config;
                          // Force reload
                          incrementConnectionKey(undefined);
                    ...
                    onRemove={() => {
                      api
                        .removeConnection(roomId, c.id)
                        .then(() => {
                          setConnections(
                            (conns) =>
                              conns?.filter((conn) => c.id !== conn.id) || [],
                          );
                          setError(null);
                        })
                        .catch((ex) => {
