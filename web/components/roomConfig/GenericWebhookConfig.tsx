...
} from "../../../src/Connections/GenericHook";
...
const DOCUMENTATION_LINK =
  "https://matrix-org.github.io/matrix-hookshot/latest/setup/webhooks.html#script-api";
  ...
  return (
    <Suspense fallback={<LoadingSpinner />}>
      ...
      <p>
        {" "}
        See the{" "}
        <a target="_blank" rel="noopener noreferrer" href={DOCUMENTATION_LINK}>
          documentation
        </a>{" "}
        for help writing transformation functions{" "}
      </p>
    </Suspense>
  );
};
...
const ConnectionConfiguration: FunctionComponent<
  ConnectionConfigurationProps<
    ...
    GenericHookConnectionState
  >
> = ({ serviceConfig, existingConnection, onSave, onRemove, isUpdating }) => {
  const [transFn, setTransFn] = useState<string>(
    (existingConnection?.config.transformationFunction as string) ||
      EXAMPLE_SCRIPT,
  );
  const [transFnEnabled, setTransFnEnabled] = useState(
    serviceConfig.allowJsTransformationFunctions &&
      !!existingConnection?.config.transformationFunction,
  );
  const [waitForComplete, setWaitForComplete] = useState(
    existingConnection?.config.waitForComplete ?? false,
  );

  const minExpiryTime = format(
    add(new Date(), { hours: 1 }),
    "yyyy-MM-dd'T'HH:mm",
  );
  const maxExpiryTime = serviceConfig.maxExpiryTime
    ? format(Date.now() + serviceConfig.maxExpiryTime, "yyyy-MM-dd'T'HH:mm")
    : undefined;
    ...
    (evt: Event) => {
      ...
      onSave({
        name:
          ...
          existingConnection?.config.name ||
          "Generic Webhook",
        expirationDate: expiryRef?.current?.value
          ? new Date(expiryRef?.current?.value).toISOString()
          : undefined,
        waitForComplete,
  ...
  const hasExpired = existingConnection?.secrets?.timeRemainingMs
    ? existingConnection?.secrets?.timeRemainingMs <= 0
    : false;
  const willExpireSoon =
    !hasExpired && existingConnection?.secrets?.timeRemainingMs
      ? existingConnection?.secrets?.timeRemainingMs <= EXPIRY_WARN_AT_MS
      : false;
  ...
  useEffect(() => {
    ...
    expiryRef.current.valueAsDate = new Date(
      existingConnection.config.expirationDate,
    );
  ...
  return (
    <form onSubmit={handleSave}>
      ...
      {willExpireSoon && (
        <Alert type="info" title="This Webhook is expiring soon">
          This Webhook will expired soon will no longer handle any incoming
          requests. To extend the Webhook lifetime, set a new expiry date below.
        </Alert>
      )}
      <InputField
      ...
      >
        <input
          ...
          type="text"
          value={existingConnection?.config.name}
        />
      </InputField>
      ...
      <InputField visible={!!existingConnection} label="URL" noPadding={true}>
        <input
          ...
          type="text"
          value={existingConnection?.secrets?.url?.toString() || ""}
        />
      </InputField>
      ...
      <InputField label="Expiration date" noPadding={true}>
        ...
        <Button
          ...
          disabled={!!expiryRef.current?.value}
        >
          Clear
        </Button>
      </InputField>
      ...
      <InputField
        visible={serviceConfig.allowJsTransformationFunctions}
        label="Enable Transformation JavaScript"
        noPadding={true}
      >
        <ToggleInput
          disabled={!canEdit}
      ...
      <InputField
        visible={serviceConfig.allowJsTransformationFunctions && transFnEnabled}
        label="Respond after function completes"
        noPadding={true}
      >
        <ToggleInput
      ...
      <ButtonSet>
        {canEdit && (
          <Button disabled={isUpdating} type="submit">
            {existingConnection ? "Save" : "Add Webhook"}
          </Button>
        )}
...
const RoomConfigListItemFunc = (c: GenericHookResponseItem) => {
  const hasExpired = c.secrets?.timeRemainingMs
    ? c.secrets.timeRemainingMs <= 0
    : false;
  const willExpireSoon =
    !hasExpired && c.secrets?.timeRemainingMs
      ? c.secrets?.timeRemainingMs <= EXPIRY_WARN_AT_MS
      : false;
