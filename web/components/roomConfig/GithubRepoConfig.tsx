...
> = ({
  ...
  const [connectionState, setConnectionState] =
    useState<GitHubRepoConnectionState | null>(null);

  const canEdit =
    !existingConnection?.id || (existingConnection?.canEdit ?? false);
  ...
  const getInstances = useMemo(
    () => async () => {
      const targets =
        await api.getConnectionTargets<GitHubRepoConnectionOrgTarget>(
          EventType,
          {},
        );
      return targets;
  ...
  const getProjects = useMemo(
    () =>
      async (
        ...
        const targets =
          await api.getConnectionTargets<GitHubRepoConnectionRepoTarget>(
            EventType,
            {
              ...(search && { search }),
              orgName: instance,
  ...
  return (
    <form onSubmit={handleSave}>
      ...
      {!existingConnection && consideredAuthenticated && (
        <ConnectionSearch
          serviceName="GitHub"
      ...
      <InputField
        visible={!!existingConnection || !!connectionState}
      ...
      >
        ...
        <ul>
          ...
          <ul>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="issue.comment"
              hookEventName="issue.comment.created"
              onChange={toggleEnabledHook}
            >
              Created
            </EventHookCheckbox>
          ...
          <ul>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="pull_request"
              hookEventName="pull_request.opened"
              onChange={toggleEnabledHook}
            >
              Opened
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="pull_request"
              hookEventName="pull_request.closed"
              onChange={toggleEnabledHook}
            >
              Closed
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="pull_request"
              hookEventName="pull_request.merged"
              onChange={toggleEnabledHook}
            >
              Merged
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="pull_request"
              hookEventName="pull_request.ready_for_review"
              onChange={toggleEnabledHook}
            >
              Ready for review
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="pull_request"
              hookEventName="pull_request.reviewed"
              onChange={toggleEnabledHook}
            >
              Reviewed
            </EventHookCheckbox>
          ...
          <ul>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.success"
              onChange={toggleEnabledHook}
            >
              Success
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.failure"
              onChange={toggleEnabledHook}
            >
              Failed
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.neutral"
              onChange={toggleEnabledHook}
            >
              Neutral
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.cancelled"
              onChange={toggleEnabledHook}
            >
              Cancelled
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.timed_out"
              onChange={toggleEnabledHook}
            >
              Timed Out
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.action_required"
              onChange={toggleEnabledHook}
            >
              Action Required
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="workflow.run"
              hookEventName="workflow.run.stale"
              onChange={toggleEnabledHook}
            >
              Stale
            </EventHookCheckbox>
      ...
      <ButtonSet>
        {canEdit && consideredAuthenticated && (
          <Button
            type="submit"
            disabled={isUpdating || (!existingConnection && !connectionState)}
          >
            {existingConnection?.id ? "Save" : "Add repository"}
          </Button>
        )}
...
const RoomConfigListItemFunc = (c: GitHubRepoResponseItem) =>
  getRepoFullName(c.config);

const GithubRepoConfig: BridgeConfig = ({ roomId, showHeader }) => {
  return (
    <RoomConfig<never, GitHubRepoResponseItem, GitHubRepoConnectionState>
