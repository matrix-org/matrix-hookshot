...
import {
  FeedConnectionState,
  FeedResponseItem,
} from "../../../src/Connections/FeedConnection";
import {
...
const FeedRecentResults: FunctionComponent<{ item: FeedResponseItem }> = ({
  item,
}) => {
  if (!item.secrets) {
    return null;
  ...
  return (
    <>
      ...
      {!item.secrets.lastResults.length && (
        <span>There have been no recent updates for this feed.</span>
      )}
      <ul>
        {item.secrets.lastResults.map((item) => (
          <li className={styles.resultListItem} key={item.timestamp}>
...
const DOCUMENTATION_LINK =
  "https://matrix-org.github.io/matrix-hookshot/latest/setup/feeds.html#feed-templates";
  ...
  return (
    <form onSubmit={handleSave}>
      ...
      <InputField visible={true} label="URL" noPadding={true}>
        <input
          ref={urlRef}
          disabled={!canEdit || !!existingConnection}
          type="text"
          value={existingConnection?.config.url}
        />
      </InputField>
      <InputField visible={true} label="Label" noPadding={true}>
        <input
          ...
          disabled={!canSave}
          type="text"
          value={existingConnection?.config.label}
        />
      </InputField>
      <InputField
        visible={onlyVisibleOnExistingConnection}
        label="Send a notice on read failure"
        noPadding={true}
      >
        <input
          disabled={!canSave}
          ...
          onChange={useCallback(() => setNotifyOnFailure((v) => !v), [])}
        />
      </InputField>
      <InputField
        visible={onlyVisibleOnExistingConnection}
      ...
      >
        <input
          ...
          disabled={!canSave}
          type="text"
          value={existingConnection?.config.template}
      ...
      <ButtonSet>
        {canSave && (
          <Button type="submit" disabled={isUpdating}>
            {existingConnection?.id ? "Save" : "Subscribe"}
          </Button>
        )}
        {canEdit && existingConnection?.id && (
          <Button intent="remove" onClick={onRemove} disabled={isUpdating}>
            Unsubscribe
          </Button>
        )}
      </ButtonSet>
...
const RoomConfigListItemFunc = (c: FeedResponseItem) =>
  c.config.label || c.config.url;

const FeedsConfig: BridgeConfig = ({ roomId, showHeader }) => {
  return (
    <RoomConfig<ServiceConfig, FeedResponseItem, FeedConnectionState>
      headerImg={FeedsIcon}
