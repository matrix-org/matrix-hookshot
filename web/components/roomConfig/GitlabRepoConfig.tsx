...
const ConnectionConfiguration: FunctionComponent<
  ConnectionConfigurationProps<
    ...
    GitLabRepoConnectionState
  >
> = ({ existingConnection, onSave, onRemove, isUpdating }) => {
  ...
  const [newConnectionState, setNewConnectionState] =
    useState<GitLabRepoConnectionState | null>(null);

  const canEdit = useMemo(
    () => !existingConnection || (existingConnection?.canEdit ?? false),
    [existingConnection],
  );
  ...
  const getInstances = useMemo(
    () => async () => {
      const targets =
        await api.getConnectionTargets<GitLabRepoConnectionInstanceTarget>(
          EventType,
          {},
        );
      return targets;
  ...
  const getProjects = useMemo(
    () =>
      async (
        ...
        const targets =
          await api.getConnectionTargets<GitLabRepoConnectionProjectTarget>(
            EventType,
            {
              instance,
              ...(search && { search }),
    ...
    <form onSubmit={handleSave}>
      ...
      >
        <input
          disabled={true}
          type="text"
          value={existingConnection?.config.instance}
        />
      </InputField>
      <InputField
        visible={!!existingConnection}
      ...
      >
        <input
          disabled={true}
          type="text"
          value={existingConnection?.config.path}
        />
      </InputField>
      <InputField
        visible={!!existingConnection || !!newConnectionState}
      ...
      >
        ...
        <ul>
          ...
          <ul>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.open"
              onChange={toggleEnabledHook}
            >
              Opened
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.reopen"
              onChange={toggleEnabledHook}
            >
              Reopened
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.close"
              onChange={toggleEnabledHook}
            >
              Closed
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.merge"
              onChange={toggleEnabledHook}
            >
              Merged
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.review"
              onChange={toggleEnabledHook}
            >
              Completed review
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.review.individual"
              onChange={toggleEnabledHook}
            >
              Single review
            </EventHookCheckbox>
            <EventHookCheckbox
              enabledHooks={enabledHooks}
              parentEvent="merge_request"
              hookEventName="merge_request.ready_for_review"
              onChange={toggleEnabledHook}
            >
              Ready for review
            </EventHookCheckbox>
      ...
      <ButtonSet>
        {canEdit && (
          <Button
            type="submit"
            disabled={
              isUpdating || (!existingConnection && !newConnectionState)
            }
          >
            {existingConnection ? "Save" : "Add project"}
          </Button>
        )}
...
const GitlabRepoConfig: BridgeConfig = ({ roomId, showHeader }) => {
  return (
    <RoomConfig<never, GitLabRepoResponseItem, GitLabRepoConnectionState>
