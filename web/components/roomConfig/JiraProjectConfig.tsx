...
> = ({ existingConnection, onSave, onRemove, isUpdating }) => {
  ...
  const [newConnectionState, setNewConnectionState] =
    useState<JiraProjectConnectionState | null>(null);
  ...
  const getInstances = useMemo(
    () => async () => {
      const targets =
        await api.getConnectionTargets<JiraProjectConnectionInstanceTarget>(
          EventType,
          {},
        );
      return targets;
  ...
  const getProjects = useMemo(
    () =>
      async (
        ...
        const targets =
          await api.getConnectionTargets<JiraProjectConnectionProjectTarget>(
            EventType,
            {
              instanceName,
              ...(search && { search }),
      ...
      >
        ...
        <ul>
          ...
          <ul>
            ...
            <EventHookCheckbox
              enabledHooks={allowedEvents}
              hookEventName="version_released"
              onChange={toggleEvent}
            >
              Released
            </EventHookCheckbox>
...
const JiraProjectConfig: BridgeConfig = ({ roomId, showHeader }) => {
  return (
    <RoomConfig<never, JiraProjectResponseItem, JiraProjectConnectionState>
      headerImg={JiraIcon}
