...
export const EventHookList: FunctionComponent<{
    ...
    return <InputField className={styles.root} visible={visible} label={label} noPadding={true}>
        <p>Choose which event should send a notification to the room</p>
        <ul>
            {Object.entries(eventList).map(([label, list]) => <div key={label}>
                {label}
                <ul>
                    {list.map((entry) => <EventHookCheckbox key={entry.eventName} enabledHooks={allowedEvents} hookEventName={entry.eventName} onChange={toggleEvent}>{entry.label}</EventHookCheckbox>)}
                </ul>
            </div>)}
        </ul>
    </InputField>
