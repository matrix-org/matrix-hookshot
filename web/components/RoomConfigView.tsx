...
const connections: Record<ConnectionType, IConnectionProps> = {
  ...
  [ConnectionType.Generic]: {
    ...
    description:
      "Create a webhook which can be used to connect any service to Matrix",
    icon: WebhookIcon,
    darkIcon: true,
...
export default function RoomConfigView(props: IProps) {
  const serviceScope =
    props.serviceScope && props.supportedServices[props.serviceScope]
      ? (props.serviceScope as ConnectionType)
      : null;
  const [activeConnectionType, setActiveConnectionType] =
    useState<ConnectionType | null>(serviceScope);
  ...
  if (activeConnectionType) {
    ...
    content = (
      <>
        <section>
          <h2> Integrations </h2>
          {(Object.keys(connections) as Array<ConnectionType>)
            .filter((service) => props.supportedServices[service])
      ...
      {!serviceScope && activeConnectionType && (
        <header>
          <span
            ...
            onClick={() => setActiveConnectionType(null)}
          >
