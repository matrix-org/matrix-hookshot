...
function RenderOAuth() {
  ...
  const service =
    (params.get("service") as OAuthPageParams["service"]) ?? "default";
  const error = params.get("error") as OAuthPageParams["error"];
  const errcode = params.get("errcode") as OAuthPageParams["errcode"];
  const oauthKind =
    (params.get("oauth-kind") as OAuthPageParams["oauth-kind"]) ?? "account";
  ...
  if (result === "error") {
    ...
    return (
      <>
        ...
        <p>
          You will need to wait for an administrator of the {serviceName}{" "}
          {oauthKind} instance to approve the new installation. If you think
          this is a mistake, contact the administrator of your organisation.
        </p>
      </>
    );
  ...
  return (
    <>
      ...
      <p>
        This is an unknown state, you may need to contact your systems
        administrator.
      </p>
    </>
  );
