...
export class JiraBotCommands extends AdminRoomCommandHandler {
  ...
  public async logout() {
    ...
    if (
      await this.tokenStore.clearUserToken(
        "jira",
        this.userId,
        this.config.jira.instanceName,
      )
    ) {
      return this.sendNotice(
        `Your JIRA account has been unlinked from your Matrix user.`,
      );
    }
  ...
  public async whoami() {
    ...
    const client = await this.tokenStore.getJiraForUser(
      this.userId,
    ...
    try {
      ...
      await this.sendNotice(
        `Could not request JIRA resources due to an error.`,
      );
      return;
    }
    let response =
      resources.length === 0
        ? `You do not have any instances authorised with this bot.`
