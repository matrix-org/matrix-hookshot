...
export class HookshotCloudJiraApi extends HookshotJiraApi {
  ...
  async *getAllProjects(
    ...
    do {
      ...
      response = await this.apiRequest<JiraCloudProjectSearchResponse>(
...
export class JiraCloudClient implements JiraClient {
   ...
   */
  static resourceCache = new QuickLRU<
    string,
    Promise<JiraAPIAccessibleResource[]>
  >({
  ...
  async getAccessibleResources() {
    ...
    const promise = (async () => {
      const res = await axios.get(
        `https://${this.instanceHost}/oauth/token/accessible-resources`,
        {
          headers: {
  ...
  async getClientForUrl(url: URL) {
    const resource = (await this.getAccessibleResources()).find(
      (r) => new URL(r.url).origin === url.origin,
    );
    if (!resource) {
      return null;
    }
