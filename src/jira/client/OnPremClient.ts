...
export class HookshotOnPremJiraApi extends HookshotJiraApi {
  ...
  async *getAllProjects(search?: string): AsyncIterable<JiraProject> {
    ...
    const results = (await this.genericGet(
      `project`,
    )) as JiraOnPremProjectSearchResponse;

    // Reasonable search algorithm.
    const searchTerm = search && createSearchTerm(search);
    if (searchTerm) {
      yield* results.filter(
        (p) =>
          createSearchTerm(p.name).includes(searchTerm) ||
          createSearchTerm(p.key).includes(searchTerm),
      );
      return;
    }
