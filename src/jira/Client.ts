...
export interface JiraClient {
  ...
  getClientForResource(
    res: JiraAPIAccessibleResource,
  ): Promise<HookshotJiraApi | null>;
}

export class JiraApiError extends Error {
  constructor(
    readonly errorMessages: string[],
    readonly errors: { description: string },
  ) {
    super();
  }
...
export abstract class HookshotJiraApi extends JiraApi {
  constructor(
    ...
    private readonly res: JiraAPIAccessibleResource,
  ) {
    super(options);
  }
  ...
  public abstract getAllProjects(
    query?: string,
    maxResults?: number,
  ...
  protected async apiRequest<T, R>(
    path: string,
    method: Method,
    data?: R,
  ): Promise<T> {
  ...
  async searchUsers(
    opts:
      ...
      | { username: string; maxResults?: number },
  ): Promise<JiraAccount[]> {
