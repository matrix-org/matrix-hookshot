...
export class RedisStorageContextualProvider implements IStorageProvider {
  constructor(
    ...
    protected readonly contextSuffix = "",
  ) {}
...
export class RedisStorageProvider
  extends RedisStorageContextualProvider
  implements IBridgeStorageProvider
{
  constructor(cacheConfig: BridgeConfigCache, contextSuffix = "") {
    super(new redis(cacheConfig.redisUri), contextSuffix);
    this.redis
      .expire(COMPLETED_TRANSACTIONS_KEY, COMPLETED_TRANSACTIONS_EXPIRE_AFTER)
      .catch((ex) => {
        log.warn("Failed to set expiry time on as.completed_transactions", ex);
      });
    this.redis
      .expire(STORED_FILES_KEY, STORED_FILES_EXPIRE_AFTER)
      .catch((ex) => {
  ...
  public async connect(): Promise<void> {
    ...
    try {
      await this.redis.expire(
        ...
        COMPLETED_TRANSACTIONS_EXPIRE_AFTER,
      );
    } catch (ex) {
      log.warn("Failed to set expiry time on as.completed_transactions", ex);
    }
  ...
  public async getGithubIssue(repo: string, issueNumber: string, scope = "") {
    const res = await this.redis.get(
      `${scope}:${GH_ISSUES_KEY}:${repo}/${issueNumber}`,
    );
    return res ? JSON.parse(res) : null;
  }

  public async setLastNotifCommentUrl(
    repo: string,
    issueNumber: string,
    url: string,
    scope = "",
  ...
  public async getLastNotifCommentUrl(
    repo: string,
    issueNumber: string,
    scope = "",
  ) {
    const res = await this.redis.get(
      `${scope}:${GH_ISSUES_LAST_COMMENT_KEY}:${repo}/${issueNumber}`,
    );
    return res ? res : null;
  }

  public async setPRReviewData(
    repo: string,
    issueNumber: string,
    url: string,
    scope = "",
  ...
  public async getPRReviewData(repo: string, issueNumber: string, scope = "") {
    const res = await this.redis.get(
      `${scope}:${GH_ISSUES_REVIEW_DATA_KEY}:${repo}/${issueNumber}`,
    );
    return res ? res : null;
  }
  ...
  public async setFigmaCommentEventId(
    roomId: string,
    figmaCommentId: string,
    eventId: string,
  ) {
    await this.redis.set(
      RedisStorageProvider.figmaCommentKey(roomId, figmaCommentId),
      eventId,
    );
  }
  ...
  async createSession(session: ProvisionSession) {
    await this.redis.set(
      `${WIDGET_TOKENS}${session.token}`,
      JSON.stringify({ userId: session.userId, expiresTs: session.expiresTs }),
    );
    await this.redis.sadd(
      `${WIDGET_USER_TOKENS}${session.userId}`,
      session.token,
    );
  }
  ...
  public async getGitlabDiscussionThreads(
    connectionId: string,
  ...
  public async setGitlabDiscussionThreads(
    connectionId: string,
  ...
  public async storeHoundActivity(
    challengeId: string,
  ...
  public async hasSeenHoundActivity(
    challengeId: string,
    ...activityHashes: string[]
  ): Promise<string[]> {
    let multi = this.redis.multi();
  ...
  public async storeHoundActivityEvent(
    challengeId: string,
    activityId: string,
    eventId: string,
  ): Promise<void> {
  ...
  public async setHasGenericHookWarnedExpiry(
    hookId: string,
    hasWarned: boolean,
  ): Promise<void> {
    await this.redis[hasWarned ? "sadd" : "srem"](
      GENERIC_HOOK_HAS_WARNED,
      hookId,
    );
  }
  ...
  public async getOpenProjectWorkPackageState(
    ...
    const jsonStr = await this.redis.get(
      `${OPENPROJECT_WORKPACKAGE_KEY}:${projectId}.${id}`,
    );
