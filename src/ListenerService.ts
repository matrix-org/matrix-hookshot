...
export type ResourceName = "webhooks" | "widgets" | "modules" | "metrics";
export const ResourceTypeArray: ResourceName[] = [
  "webhooks",
  "widgets",
  "modules",
  "metrics",
...
export class ListenerService {
  ...
  public getApplicationsPrefixesForResource(
    ...
    for (const listener of listeners) {
      log.debug(
        `Reverse binding ${listener.config.bindAddress || "127.0.0.1"}:${listener.config.port} for ${resourceName}`,
      );
      listener.resourcesBound = true;
    }
      ...
      if (listener.server && listener.server.listening) {
        promises.push(
          new Promise<void>((res, rej) =>
            listener.server?.close((e) => (e ? rej(e) : res())),
          ),
        );
      }
    }
