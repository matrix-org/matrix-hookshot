...
export class MatrixSender {
  ...
  public listen() {
    ...
    this.mq.on<IMatrixSendMessage>("matrix.message", async (msg) => {
      try {
        ...
        log.error(
          `Failed to send message (${msg.data.roomId}, ${msg.data.sender}, ${msg.data.type})`,
          ex,
        );
      }
    });
  ...
  public async sendMatrixMessage(messageId: string, msg: IMatrixSendMessage) {
    const intent = msg.sender
      ? this.as.getIntentForUserId(msg.sender)
      : this.as.botIntent;
    ...
    try {
      const eventId = await intent.underlyingClient.sendEvent(
        msg.roomId,
        msg.type,
        msg.content,
      );
...
export class MessageSenderClient {
  ...
  public async sendMatrixMessage(
    ...
    const result = await this.queue.pushWait<
      IMatrixSendMessage,
      IMatrixSendMessageResponse | IMatrixSendMessageFailedResponse
    >({
