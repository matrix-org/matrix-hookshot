...
export class BridgeWidgetApi extends ProvisioningApi {
  constructor(
    ...
    this.addRoute(
      "get",
      "/v1/config/sections",
      wrapHandler(this.getConfigSections),
    );
    this.addRoute(
      "get",
      "/v1/service/:service/config",
      wrapHandler(this.getServiceConfig),
    );
    this.addRoute(
      "get",
      "/v1/:roomId/connections",
      wrapHandler(this.getConnections),
    );
    this.addRoute(
      "get",
      "/v1/:roomId/connections/:service",
      wrapHandler(this.getConnectionsForService),
    );
    this.addRoute(
      "post",
      "/v1/:roomId/connections/:type",
      wrapHandler(this.createConnection),
    );
    this.addRoute(
      "put",
      "/v1/:roomId/connections/:connectionId",
      wrapHandler(this.updateConnection),
    );
    this.addRoute(
      "patch",
      "/v1/:roomId/connections/:connectionId",
    ...
    this.addRoute(
      "delete",
      "/v1/:roomId/connections/:connectionId",
      wrapHandler(this.deleteConnection),
    );
    this.addRoute(
      "get",
      "/v1/targets/:type",
      wrapHandler(this.getConnectionTargets),
    );
    this.addRoute(
      "get",
      "/v1/service/:service/auth",
      wrapHandler(this.getAuth),
    );
    this.addRoute(
      "get",
      "/v1/service/:service/auth/:state",
      wrapHandler(this.getAuthPoll),
    );
    this.addRoute(
      "post",
      "/v1/service/:service/auth/logout",
      wrapHandler(this.postAuthLogout),
    );
  }

  private async getBotUserInRoom(
    roomId: string,
    serviceType?: string,
  ): Promise<BotUser> {
    ...
    if (!botUser) {
      ...
      const intent =
        (serviceType &&
          this.botUsersManager.getBotUserForService(serviceType)?.intent) ||
        this.as.botIntent;
      try {
        ...
        throw new ApiError(
          "Bot was not invited to the room.",
          ErrCode.NotInRoom,
        );
      }
  ...
  private async getRoomFromRequest(
    req: ProvisioningRequest,
  ): Promise<AdminRoom> {
    const room = [...this.adminRooms.values()].find(
  ...
  private async getRoomState(req: ProvisioningRequest, res: Response) {
    try {
      ...
      throw new ApiError(
        "An error occurred when getting room state",
        ErrCode.Unknown,
      );
    }
  }

  private async getConfigSections(
    req: ProvisioningRequest,
    res: Response<{ [section: string]: boolean }>,
  ) {
    res.send({
      general: true,
  ...
  private async getConnections(
    req: ProvisioningRequest,
    res: Response<GetConnectionsResponseItem[]>,
  ) {
  ...
  private async getConnectionsForService(
    req: ProvisioningRequest,
    res: Response<GetConnectionsForServiceResponse<GetConnectionsResponseItem>>,
  ) {
  ...
  private async createConnection(
    ...
    const connectionType =
      this.connMan.getConnectionTypeForEventType(eventType);
    if (!connectionType) {
    ...
    try {
      ...
      const result = await this.connMan.provisionConnection(
        roomId,
        botUser.intent,
        req.userId,
        connectionType,
  ...
  private async updateConnection(
    ...
    if (
      !connection.provisionerUpdateConfig ||
      !connection.getProvisionerDetails
    ) {
      throw new ApiError(
        "Connection type does not support updates",
  ...
  private async getAuthPoll(
    req: ProvisioningRequest,
    res: Response<GetAuthPollResponse>,
  ) {
    if (!req.userId) {
      throw Error("Expected userId on request");
    }
  ...
  private async postAuthLogout(
    ...
    if (AllowedTokenTypes.includes(service)) {
      const result = await this.tokenStore.clearUserToken(
        service as TokenType,
        req.userId,
      );
      if (result) {
        res.send({ ok: true });
