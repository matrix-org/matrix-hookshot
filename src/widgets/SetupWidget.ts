...
import { BridgeWidgetConfig } from "../config/sections";
const log = new Logger("SetupWidget");

export class SetupWidget {
  static async SetupAdminRoomConfigWidget(
    roomId: string,
    botIntent: Intent,
    config: BridgeWidgetConfig,
  ): Promise<boolean> {
    if (
      await SetupWidget.createWidgetInRoom(
        roomId,
        botIntent,
        config,
        HookshotWidgetKind.Settings,
        "bridge_control",
      )
    ) {
      await botIntent.sendText(
        roomId,
        `If your client supports it, you can open the "${config.branding.widgetTitle}" widget to configure hookshot.`,
      );
      return true;
    }
    return false;
  ...
  static async SetupRoomConfigWidget(
    roomId: string,
    botIntent: Intent,
    config: BridgeWidgetConfig,
    serviceTypes: string[],
    ...
    const serviceScope =
      serviceTypes.length === 1 ? serviceTypes[0] : undefined;
    if (
      await SetupWidget.createWidgetInRoom(
        roomId,
        botIntent,
  ...
  private static async createWidgetInRoom(
    ...
    if (
      !(await botIntent.underlyingClient.userHasPowerLevelFor(
        botIntent.userId,
        roomId,
      ...
      throw new CommandError(
        ...
        "I do not have permission to create a widget in this room. Please promote me to an Admin/Moderator.",
      );
    }
    try {
    ...
    await botIntent.underlyingClient.sendStateEvent(
      ...
      stateKey,
      {
        ...
        type: "m.custom",
        url: new URL(
          `#/?kind=${kind}&roomId=$matrix_room_id&widgetId=$matrix_widget_id${serviceScope ? `&serviceScope=${serviceScope}` : ""}`,
          config.parsedPublicUrl,
        ).href,
