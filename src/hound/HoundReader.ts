...
export class HoundReader {
  ...
  constructor(
    ...
    this.connections =
      this.connectionManager.getAllConnectionsOfType(HoundConnection);
    ...
    connectionManager.on("new-connection", (newConnection) => {
      ...
      if (!this.challengeIds.includes(newConnection.challengeId)) {
        log.info(
          `Connection added, adding "${newConnection.challengeId}" to queue`,
        );
    ...
    connectionManager.on("connection-removed", (removed) => {
      ...
      this.connections = this.connections.filter((c) => {
        ...
        if (c.connectionId !== removed.connectionId) {
          shouldKeepUrl =
            shouldKeepUrl || c.challengeId === removed.challengeId;
          return true;
        }
        return false;
      });
      if (shouldKeepUrl) {
        log.info(
          `Connection removed, but not removing "${removed.challengeId}" as it is still in use`,
        );
        return;
      }
      log.info(
        `Connection removed, removing "${removed.challengeId}" from queue`,
      );
      this.challengeIds = this.challengeIds.filter(
        (u) => u !== removed.challengeId,
      );
    });
  ...
  public async poll(challengeId: string) {
    const resAct = await this.houndClient.get(
      `https://api.challengehound.com/v1/activities?challengeId=${challengeId}&size=10`,
    );
    const activites = (resAct.data["results"] as HoundActivity[]).map((a) => ({
      ...a,
      hash: HoundReader.hashActivity(a),
    }));
    const seen = await this.storage.hasSeenHoundActivity(
      challengeId,
      ...activites.map((a) => a.hash),
    );

    // Don't emit anything if our cache is empty, as we'll probably create duplicates.
    const hasSeenChallenge =
      await this.storage.hasSeenHoundChallenge(challengeId);
    if (hasSeenChallenge) {
    ...
    await this.storage.storeHoundActivity(
      challengeId,
      ...activites.filter((s) => !seen.includes(s.hash)).map((a) => a.hash),
    );
  }
  ...
  public async pollChallenges(): Promise<void> {
    ...
    if (challengeId) {
      try {
        ...
        log.debug(
          `Activity fetching took ${elapsed / 1000}s, sleeping for ${sleepFor / 1000}s`,
        );

        if (elapsed > this.sleepingInterval) {
          log.warn(
            `It took us longer to update the activities than the expected interval`,
          );
        }
      } catch (ex) {
