...
export class Bridge {
  ...
  constructor(
    ...
    this.commentProcessor = new CommentProcessor(
      this.as,
      this.config.bridge.mediaUrl || this.config.bridge.url,
    );
    this.notifProcessor = new NotificationProcessor(
      this.storage,
      this.messageClient,
    );
  ...
  public async start() {
    ...
    this.as.on("room.failed_decryption", (roomId, event, err) => {
      log.warn(
        `Failed to decrypt event ${event.event_id} from ${roomId}: ${err.message}`,
      );
    ...
    const validateRepoIssue = (
      data:
        | GitHubWebhookTypes.IssuesEvent
        | GitHubWebhookTypes.IssueCommentEvent,
    ) => {
    ...
    this.queue.on<GitHubWebhookTypes.InstallationCreatedEvent>(
      "github.installation.created",
      async (data) => {
        this.github?.onInstallationCreated(data.data);
      },
    );
    this.queue.on<GitHubWebhookTypes.InstallationUnsuspendEvent>(
      "github.installation.unsuspend",
      async (data) => {
        this.github?.onInstallationCreated(data.data);
      },
    );
    this.queue.on<GitHubWebhookTypes.InstallationDeletedEvent>(
      "github.installation.deleted",
      async (data) => {
        this.github?.onInstallationRemoved(data.data);
      },
    );
    this.queue.on<GitHubWebhookTypes.InstallationSuspendEvent>(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.IssuesOpenedEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.IssuesEditedEvent,
      GitHubIssueConnection | GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.IssuesClosedEvent,
      GitHubIssueConnection | GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.IssuesReopenedEvent,
      GitHubIssueConnection | GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.IssuesUnlabeledEvent,
      GitHubRepoConnection
    >(
      "github.issues.unlabeled",
      (data) =>
        connManager.getConnectionsForGithubRepo(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.IssuesLabeledEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.PullRequestOpenedEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.PullRequestClosedEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.PullRequestReadyForReviewEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.PullRequestReviewSubmittedEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.WorkflowRunCompletedEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.ReleasePublishedEvent,
      GitHubRepoConnection
    >(
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.ReleaseCreatedEvent,
      GitHubRepoConnection
    >(
    ...
    this.queue.on<OAuthRequest>("github.oauth.response", async (msg) => {
      const userId = this.tokenStore.getUserIdForOAuthState(
        msg.data.state,
        false,
      );
    ...
    this.queue.on<GitHubOAuthTokenResponse>(
      "github.oauth.tokens",
      async (msg) => {
        const userId = this.tokenStore.getUserIdForOAuthState(msg.data.state);
        if (!userId) {
          log.warn(
            "Could not find internal state for successful tokens request. This shouldn't happen!",
          );
          return;
        }
        await this.tokenStore.storeUserToken(
          "github",
          userId,
          JSON.stringify({
            ...
            expires_in:
              msg.data.expires_in &&
              parseInt(msg.data.expires_in) * 1000 + Date.now(),
            token_type: msg.data.token_type,
            refresh_token: msg.data.refresh_token,
            refresh_token_expires_in:
              msg.data.refresh_token_expires_in &&
              parseInt(msg.data.refresh_token_expires_in) * 1000 + Date.now(),
          } as GitHubOAuthToken),
        );
    ...
    this.bindHandlerToQueue<
      IGitLabWebhookNoteEvent,
      GitLabIssueConnection | GitLabRepoConnection
    >(
      "gitlab.note.created",
      (data) => {
      ...
      (c, data) =>
        c instanceof GitLabRepoConnection
          ? c.onMergeRequestCommentCreated(data)
          : c.onCommentCreated(data),
    );
    ...
    this.bindHandlerToQueue<
      GitHubWebhookTypes.DiscussionCommentCreatedEvent,
      GitHubDiscussionConnection
    >(
      "github.discussion_comment.created",
      (data) =>
        connManager.getConnectionsForGithubDiscussion(
          data.repository.owner.login,
          data.repository.name,
          data.discussion.number,
        ),
      (c, data) => c.onDiscussionCommentCreated(data),
    );

    this.queue.on<GitHubWebhookTypes.DiscussionCreatedEvent>(
      "github.discussion.created",
      async ({ data }) => {
        if (!this.github || !this.config.github) {
          return;
        }
        const spaces = connManager.getConnectionsForGithubRepoDiscussion(
          data.repository.owner.login,
          data.repository.name,
        );
        if (spaces.length === 0) {
          log.info(
            `Not creating discussion ${data.discussion.id} ${data.repository.owner.login}/${data.repository.name}, no target spaces`,
          );
          // We don't want to create any discussions if we have no target spaces.
          return;
        }
        let [discussionConnection] =
          connManager.getConnectionsForGithubDiscussion(
            data.repository.owner.login,
            data.repository.name,
            data.discussion.id,
        ...
        if (!discussionConnection) {
          const botUser = this.botUsersManager.getBotUserForService(
            GitHubDiscussionConnection.ServiceCategory,
          );
          if (!botUser) {
          ...
          try {
            ...
            discussionConnection =
              await GitHubDiscussionConnection.createDiscussionRoom(
                this.as,
                botUser.intent,
                null,
                data.repository.owner.login,
        ...
        spaces.map(async (c) => {
          try {
            ...
            log.warn(
              `Failed to add discussion ${c.toString()} failed to handle comment.created:`,
              ex,
            );
          }
        });
    ...
    this.bindHandlerToQueue<JiraIssueEvent, JiraProjectConnection>(
      "jira.issue_created",
      (data) =>
        connManager.getConnectionsForJiraProject(data.issue.fields.project),
    ...
    this.queue.on<OAuthRequest | JiraOAuthRequestOnPrem>(
      "jira.oauth.response",
      async (msg) => {
        ...
        try {
          ...
          if ("code" in msg.data) {
            tokenInfo = await this.tokenStore.jiraOAuth.exchangeRequestForToken(
              msg.data.code,
            );
          } else {
            tokenInfo = await this.tokenStore.jiraOAuth.exchangeRequestForToken(
              msg.data.oauthToken,
              msg.data.oauthVerifier,
            );
          }
    ...
    this.queue.on<OAuthRequest>("openproject.oauth.response", async (msg) => {
      ...
      const result = await this.tokenStore.openProjectOAuth.handleOAuth(
        msg.data,
      );
      await this.queue.push<OAuthRequestResult>({
        data: result,
    ...
    this.queue.on<GenericWebhookEvent>("generic-webhook.event", async (msg) => {
      ...
      const connections = connManager.getConnectionsForGenericWebhook(
        data.hookId,
      );
      log.debug(
        `generic-webhook.event for ${connections.map((c) => c.toString()).join(", ") || "[empty]"}`,
      );
    ...
    this.bindHandlerToQueue<FigmaEvent, FigmaFileConnection>(
      "figma.payload",
      (data) =>
        connManager.getForFigmaFile(data.payload.file_key, data.instanceName),
    ...
    >(
      ...
      (data) =>
        connManager.getConnectionsForOpenProject(
    ...
    const processRooms = async () => {
      for (
        ...
        if (!botUser) {
          log.error(
            `Failed to find a bot in room '${roomId}' when setting up admin room`,
          );
          continue;
        }
        ...
        try {
          let accountData =
            await botUser.intent.underlyingClient.getSafeRoomAccountData<AdminAccountData>(
              BRIDGE_ROOM_TYPE,
              roomId,
            );
          if (!accountData) {
            ...
            if (!accountData) {
              log.debug(
                `Room ${roomId} has no connections and is not an admin room`,
              );
              continue;
            } else {
              // Upgrade the room
          ...
          try {
            notifContent =
              await botUser.intent.underlyingClient.getRoomStateEvent(
                roomId,
                NotifFilter.StateType,
                "",
              );
          } catch {
            try {
          ...
          const adminRoom = await this.setUpAdminRoom(
            botUser.intent,
            roomId,
            accountData,
            notifContent || NotifFilter.getDefaultContent(),
          );
          // Call this on startup to set the state
          await this.onAdminRoomSettingsChanged(adminRoom, accountData, {
            admin_user: accountData.admin_user,
          });
    ...
    // Handle spaces
    for (const discussion of connManager.getAllConnectionsOfType(
      GitHubDiscussionSpace,
    )) {
  ...
  private async handleHookshotEvent<EventType, ConnType extends IConnection>(
    msg: MessageQueueMessageOut<EventType>,
    connection: ConnType,
    handler: (c: ConnType, data: EventType) => Promise<unknown> | unknown,
  ) {
    try {
      ...
      Sentry.withScope((scope) => {
        ...
        log.warn(
          `Connection ${connection.toString()} failed to handle ${msg.eventName}:`,
          e,
        );
        Metrics.connectionsEventFailed.inc({
          event: msg.eventName,
  ...
  private async bindHandlerToQueue<EventType, ConnType extends IConnection>(
    event: string,
    connectionFetcher: (data: EventType) => ConnType[],
    handler: (c: ConnType, data: EventType) => Promise<unknown> | unknown,
  ) {
    ...
    this.queue.on<EventType>(event, (msg) => {
      ...
      log.debug(
        `${event} for ${connections.map((c) => c.toString()).join(", ") || "[empty]"}`,
      );
  ...
  private async onRoomInvite(
    ...
    if (
      !this.config.checkPermissionAny(event.sender, BridgePermissionLevel.login)
    ) {
      return botUser.intent.underlyingClient.leaveRoom(
        roomId,
    ...
    if (event.content.is_direct && botUser.userId !== this.as.botUserId) {
      ...
      log.debug(
        `Rejecting direct message (admin room) invite to room ${roomId} for service bot ${botUser.userId}`,
      );
      return botUser.intent.underlyingClient.leaveRoom(
        roomId,
        "This bot does not support admin rooms.",
      );
    }
    ...
    if (event.content.is_direct) {
      await botUser.intent.underlyingClient.setRoomAccountData(
        BRIDGE_ROOM_TYPE,
        roomId,
        { admin_user: event.sender },
      );
    }
  }

  private async onRoomLeave(
    roomId: string,
    ...
    if (!adminRoom) {
      ...
      for (const connection of this.connectionManager.getAllConnectionsForRoom(
        ...
        try {
          ...
          log.warn(
            `Connection ${connection.toString()} failed to handle message:`,
            ex,
          );
      ...
      if (
        ...
        for (const botUser of botUsersInRoom) {
          try {
            ...
            const handled = await setupConnection.onMessageEvent(
              event,
              checkPermission,
            );
            if (handled) {
    ...
    if (replyEvent) {
      ...
      try {
        ...
        if (splitParts && issueNumber) {
          ...
          const connections =
            this.connectionManager.getConnectionsForGithubIssue(
              splitParts[0],
              splitParts[1],
              issueNumber,
            );
        ...
        await adminRoom.sendNotice(
          "Failed to handle reply. You may not be authenticated to do that.",
        );
  ...
  private async onRoomJoin(
    ...
    if (adminAccountData) {
      const room = await this.setUpAdminRoom(
        botUser.intent,
        roomId,
        adminAccountData,
    ...
    if (
      !roomHasConnection &&
      !adminAccountData &&
      this.config.widgets?.roomSetupWidget?.addOnInvite
    ) {
      try {
        const hasPowerlevel =
          await botUser.intent.underlyingClient.userHasPowerLevelFor(
        ...
        if (!hasPowerlevel) {
          await botUser.intent.sendText(
            roomId,
            "Hello! To set up new integrations in this room, please promote me to a Moderator/Admin.",
          );
        } else {
          // Set up the widget
          await SetupWidget.SetupRoomConfigWidget(
            roomId,
            botUser.intent,
            this.config.widgets,
            botUser.services,
  ...
  private async onRoomEvent(
    roomId: string,
    event: MatrixEvent<Record<string, unknown>>,
  ) {
    if (!this.connectionManager) {
      // Not ready yet.
      return;
    ...
    if (event.state_key !== undefined) {
      ...
      for (const connection of existingConnections) {
        if (
          !this.connectionManager.verifyStateEventForConnection(
            connection,
            state,
            true,
          )
        ...
        try {
          ...
          if (
            event.content.disabled === true ||
            Object.keys(event.content).length === 0
          ) {
            await this.connectionManager.purgeConnection(
              connection.roomId,
          ...
          try {
            await this.as.botClient.sendReadReceipt(
              connection.roomId,
              event.event_id,
            );
          } catch {
          ...
          log.warn(
            `Connection ${connection.toString()} for ${roomId} failed to handle state update:`,
            ex,
          );
        }
      }
      if (!existingConnections.length) {
        ...
        try {
          const connection =
            await this.connectionManager.createConnectionForState(
              roomId,
          ...
          if (connection) {
            log.info(
              `New connected added to ${roomId}: ${connection.toString()}`,
            );
            this.connectionManager.push(connection);
          }
        } catch (ex) {
          log.error(
            `Failed to handle connection for state ${event.type} in ${roomId}`,
            ex,
          );
        }
      }
      ...
      for (const botUser of botUsersInRoom) {
        ...
        if (
          ...
          if (currentPl !== previousPl && currentPl >= requiredPl) {
            ...
            try {
              log.info(
                `Bot has powerlevel required to create a setup widget, attempting`,
              );
              await SetupWidget.SetupRoomConfigWidget(
                roomId,
                botUser.intent,
    ...
    for (const connection of this.connectionManager.getAllConnectionsForRoom(
      ...
      try {
        ...
        Sentry.captureException(ex, scope);
        log.warn(
          `Connection ${connection.toString()} failed to handle onEvent:`,
          ex,
        );
      }
    }
  ...
  private async onAdminRoomSettingsChanged(
    adminRoom: AdminRoom,
    settings: AdminAccountData,
    oldSettings: AdminAccountData,
  ) {
    log.debug(`Settings changed for ${adminRoom.userId}`, settings);
    // Make this more efficent.
    if (
      !oldSettings.github?.notifications?.enabled &&
      settings.github?.notifications?.enabled
    ) {
      log.info(`Notifications enabled for ${adminRoom.userId}`);
      const token = await this.tokenStore.getGitHubToken(adminRoom.userId);
      if (token) {
        log.info(
          `Notifications enabled for ${adminRoom.userId} and token was found`,
        );
        ...
        log.warn(
          `Notifications enabled for ${adminRoom.userId} but no token stored!`,
        );
      }
    } else if (
      oldSettings.github?.notifications?.enabled &&
    ...
    for (const [instanceName, instanceSettings] of Object.entries(
      settings.gitlab || {},
    )) {
      ...
      if (token && instanceSettings.notifications.enabled) {
        log.info(
          `GitLab ${instanceName} notifications enabled for ${adminRoom.userId}`,
        );
        await this.queue.push<NotificationsEnableEvent>({
          eventName: "notifications.user.enable",
          sender: "Bridge",
        ...
        log.info(
          `GitLab ${instanceName} notifications disabled for ${adminRoom.userId}`,
        );
        await this.queue.push<NotificationsDisableEvent>({
          eventName: "notifications.user.disable",
          sender: "Bridge",
  ...
  private async getOrCreateAdminRoom(
    intent: Intent,
    userId: string,
  ): Promise<AdminRoom> {
  ...
  private async setUpAdminRoom(
    ...
    adminRoom.on(
      "settings.changed",
      this.onAdminRoomSettingsChanged.bind(this),
    );
    adminRoom.on("open.project", async (project: ProjectsGetResponseData) => {
      const [connection] =
        this.connectionManager?.getForGitHubProject(project.id) || [];
      if (!connection) {
    ...
    adminRoom.on(
      "open.gitlab-issue",
      async (
        issueInfo: GetIssueOpts,
        res: GetIssueResponse,
        instanceName: string,
        ...
        const [connection] =
          this.connectionManager?.getConnectionsForGitLabIssue(
            instance,
            issueInfo.projects,
            issueInfo.issue,
          ) || [];
        if (connection) {
          return intent.underlyingClient.inviteUser(
            adminRoom.userId,
            connection.roomId,
          );
        }
  ...
  private onTokenUpdated(
    ...
    if (type === "gitlab") {
      ...
      instanceName = Object.entries(this.config.gitlab?.instances || {}).find(
        (i) => i[1].url === instanceUrl,
      )?.[0];
    } else if (type === "github") {
    ...
    for (const adminRoom of this.adminRooms.values()) {
      ...
      if (adminRoom?.notificationsEnabled(type, instanceName)) {
        log.debug(
          `Token was updated for ${userId} (${type}), notifying notification watcher`,
        );
        this.queue
          .push<NotificationsEnableEvent>({
            eventName: "notifications.user.enable",
            sender: "Bridge",
            data: {
              userId: adminRoom.userId,
              ...
              token,
              filterParticipating:
                adminRoom.notificationsParticipating("github"),
              type,
              instanceUrl,
            },
          })
          .catch((ex) =>
            log.error(`Failed to push notifications.user.enable:`, ex),
          );
      }
    }
  }
