 ...
 */
@Connection
export class GitHubProjectConnection
  ...
  implements IConnection
{
  ...
  public static createConnectionForState(
    ...
    { config, as, intent }: InstantiateConnectionOpts,
  ) {
    if (!config.github) {
  ...
  static async onOpenProject(
    project: ProjectsGetResponseData,
    as: Appservice,
    intent: Intent,
    config: BridgeConfig,
    inviteUser: string,
  ...
  public isInterestedInStateEvent(eventType: string, stateKey: string) {
    return (
      GitHubProjectConnection.EventTypes.includes(eventType) &&
