 ...
 */
@Connection
export class OutboundHookConnection
  ...
  implements IConnection
{
  ...
  static validateState(
    ...
    throw new ApiError(
      "Outbound Method must be one of PUT,POST",
      ErrCode.BadValue,
    );
  }
  ...
  static async createConnectionForState(
    ...
    const token = await tokenStore.getGenericToken(
      "outboundHookToken",
      hashId(`${roomId}:${event.stateKey}`),
    );
  ...
  static async provisionConnection(
    ...
    const connection = new OutboundHookConnection(
      roomId,
      validState,
      token,
      stateKey,
  ...
  public isInterestedInStateEvent(eventType: string, stateKey: string) {
    return (
      OutboundHookConnection.EventTypes.includes(eventType) &&
  ...
  ): Promise<{ blob: Blob; event: MatrixEvent<unknown> } | null> {
    // Check for non-extendable event types first.
    const content = ev.content as FileMessageEventContent;

    if (
      !["m.image", "m.audio", "m.file", "m.video"].includes(content.msgtype)
    ) {
      return null;
    }
  ...
  public async onEvent(ev: MatrixEvent<unknown>): Promise<void> {
    ...
    try {
      ...
      if (mediaResult) {
        multipartBlob.set(
          "event",
          new Blob([JSON.stringify(mediaResult?.event)], {
