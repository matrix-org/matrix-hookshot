...
export class FigmaFileConnection extends BaseConnection implements IConnection {
  static readonly CanonicalEventType =
    "uk.half-shot.matrix-hookshot.figma.file";
  ...
  public static createConnectionForState(
    ...
    { config, as, intent, storage }: InstantiateConnectionOpts,
  ) {
    if (!config.figma) {
      throw Error("Figma is not configured");
    }
    return new FigmaFileConnection(
      roomId,
      event.stateKey,
      event.content,
      config,
  ...
  static async provisionConnection(
    ...
    userId: string,
    data: Record<string, unknown> = {},
    { as, intent, config, storage }: ProvisionConnectionOpts,
  ) {
    if (!config.figma) {
      throw Error("Figma is not configured");
    }
    const validState = this.validateState(data);
    const connection = new FigmaFileConnection(
      roomId,
      validState.fileId,
      validState,
      config,
    ...
    await new GrantChecker(as.botIntent, "figma").grantConnection(roomId, {
      fileId: validState.fileId,
      instanceName: validState.instanceName || "none",
    });
  ...
  public async handleNewComment(payload: FigmaPayload) {
    ...
    if (age > 5000) {
      ...
      log.warn(
        `Comment ${payload.comment_id} is stale, ignoring (${age}ms old)`,
      );
      return;
    }
    ...
    const name = payload.triggered_by.handle
      .split(" ")
      .map((p) => p[0] + empty + p.slice(1))
      .join(" ");
    let content: Record<string, unknown> | undefined = undefined;
    const parentEventId =
      payload.parent_id &&
      (await this.storage.getFigmaCommentEventId(
        this.roomId,
        payload.parent_id,
      ));
    if (parentEventId) {
    ...
    content["uk.half-shot.matrix-hookshot.figma.comment_id"] =
      payload.comment_id;
    const eventId = await intent.sendEvent(this.roomId, content);
    log.info(
      `New figma comment ${payload.comment_id} -> ${this.roomId}/${eventId}`,
    );
    await this.storage.setFigmaCommentEventId(
      this.roomId,
      payload.comment_id,
      eventId,
    );
