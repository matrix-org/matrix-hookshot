...
export class SetupConnection extends CommandConnection {
  ...
  constructor(
    ...
    private readonly getOrCreateAdminRoom: (
      intent: Intent,
      userId: string,
    ) => Promise<AdminRoom>,
  ...
  @botCommand("github repo", {
    help: "Create a connection for a GitHub repository. (You must be logged in with GitHub to do this.)",
    requiredArgs: ["url"],
    includeUserId: true,
    category: GitHubRepoConnection.ServiceCategory,
  })
  public async onGitHubRepo(userId: string, url: string) {
    if (!this.provisionOpts.github || !this.config.github) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support GitHub.",
      );
    }
    ...
    const octokit =
      await this.provisionOpts.tokenStore.getOctokitForUser(userId);
    if (!octokit) {
      throw new CommandError(
        "User not logged in",
        "You are not logged into GitHub. Start a DM with this bot and use the command `github login`.",
      );
    }
    const urlParts =
    ...
    if (!urlParts) {
      throw new CommandError(
        "Invalid GitHub url",
        "The GitHub url you entered was not valid.",
      );
    }
    const [, org, repo] = urlParts;
    const { connection } = await GitHubRepoConnection.provisionConnection(
      this.roomId,
      userId,
      { org, repo },
      this.provisionOpts,
    ...
    await this.client.sendNotice(
      this.roomId,
      `Room configured to bridge ${connection.org}/${connection.repo}`,
    );
  }

  @botCommand("gitlab project", {
    help: "Create a connection for a GitHub project. (You must be logged in with GitLab to do this.)",
    requiredArgs: ["url"],
    includeUserId: true,
    category: GitLabRepoConnection.ServiceCategory,
  })
  public async onGitLabRepo(userId: string, url: string) {
    if (!this.config.gitlab) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support GitLab.",
      );
    }

    await this.checkUserPermissions(
      userId,
      GitLabRepoConnection.ServiceCategory,
      GitLabRepoConnection.CanonicalEventType,
    );

    const { name, instance } =
      this.config.gitlab.getInstanceByProjectUrl(url) || {};
    if (!instance || !name) {
      throw new CommandError(
        "not-configured",
        "No instance found that matches the provided URL.",
      );
    }

    const client = await this.provisionOpts.tokenStore.getGitLabForUser(
      userId,
      instance.url,
    );
    if (!client) {
      throw new CommandError(
        "User not logged in",
        "You are not logged into this GitLab instance. Start a DM with this bot and use the command `gitlab personaltoken`.",
      );
    }
    ...
    if (!path) {
      throw new CommandError(
        "Invalid GitLab url",
        "The GitLab project url you entered was not valid.",
      );
    }
    const { connection, warning } =
      await GitLabRepoConnection.provisionConnection(
        this.roomId,
        userId,
        { path, instance: name },
        this.provisionOpts,
    ...
    await this.client.sendNotice(
      this.roomId,
      `Room configured to bridge ${connection.prettyPath}` +
        (warning ? `\n${warning.header}: ${warning.message}` : ""),
    );
  }

  private async checkJiraLogin(userId: string, urlStr: string) {
    const jiraClient = await this.provisionOpts.tokenStore.getJiraForUser(
      userId,
      urlStr,
    );
    if (!jiraClient) {
      throw new CommandError(
        "User not logged in",
        "You are not logged into Jira. Start a DM with this bot and use the command `jira login`.",
      );
    }
  }
  ...
  private async getJiraProjectSafeUrl(urlStr: string) {
    ...
    if (!projectKey) {
      throw new CommandError(
        "Invalid Jira url",
        "The JIRA project url you entered was not valid. It should be in the format of `https://jira-instance/.../projects/PROJECTKEY/...` or `.../RapidBoard.jspa?projectKey=TEST`.",
      );
    }
    return `https://${url.host}/projects/${projectKey}`;
  }

  @botCommand("jira project", {
    help: "Create a connection for a JIRA project. (You must be logged in with JIRA to do this.)",
    requiredArgs: ["url"],
    includeUserId: true,
    category: JiraProjectConnection.ServiceCategory,
  })
  public async onJiraProject(userId: string, urlStr: string) {
    if (!this.config.jira) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support Jira.",
      );
    }

    await this.checkUserPermissions(
      userId,
      JiraProjectConnection.ServiceCategory,
    ...
    const res = await JiraProjectConnection.provisionConnection(
      this.roomId,
      userId,
      { url: safeUrl },
      this.provisionOpts,
    ...
    await this.client.sendNotice(
      this.roomId,
      `Room configured to bridge Jira project ${res.connection.projectKey}.`,
    );
  }

  @botCommand("jira list project", {
    help: "Show JIRA projects currently connected to.",
    category: JiraProjectConnection.ServiceCategory,
  })
  public async onJiraListProject() {
    const projects: JiraProjectConnectionState[] = await this.client
      .getRoomState(this.roomId)
      .catch((err: any) => {
    ...
    if (projects.length === 0) {
      return this.client.sendHtmlNotice(
        this.roomId,
        md.renderInline("Not connected to any JIRA projects"),
      );
    } else {
      return this.client.sendHtmlNotice(
        this.roomId,
  ...
  @botCommand("jira remove project", {
    help: "Remove a connection for a JIRA project.",
    requiredArgs: ["url"],
    includeUserId: true,
    category: JiraProjectConnection.ServiceCategory,
  })
  public async onJiraRemoveProject(userId: string, urlStr: string) {
    ...
    for (eventType of eventTypes) {
      try {
        event = await this.client.getRoomStateEvent(
          this.roomId,
          eventType,
          safeUrl,
        );
    ...
    if (!event || Object.keys(event).length === 0) {
      throw new CommandError(
        "Invalid Jira project URL",
        `Feed "${urlStr}" is not currently bridged to this room`,
      );
    }
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Room no longer bridged to Jira project \`${safeUrl}\`.`),
    );
  }

  @botCommand("webhook", {
    help: "Create an inbound webhook. The liveDuration must be specified as a duration string (e.g. 30d).",
    requiredArgs: ["name"],
    includeUserId: true,
    optionalArgs: ["liveDuration"],
    category: GenericHookConnection.ServiceCategory,
  })
  public async onWebhook(userId: string, name: string, liveDuration?: string) {
    if (!this.config.generic?.enabled) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support webhooks.",
      );
    }

    let expirationDate: string | undefined = undefined;
    if (liveDuration) {
      const expirationDuration = await (
        await parseDurationImport
      ).default(liveDuration);
      if (!expirationDuration) {
        throw new CommandError(
          "Bad webhook duration",
    ...
    if (!name || name.length < 3 || name.length > 64) {
      throw new CommandError(
        "Bad webhook name",
        "A webhook name must be between 3-64 characters.",
      );
    }
    const c = await GenericHookConnection.provisionConnection(
      this.roomId,
      userId,
      { name, expirationDate },
      this.provisionOpts,
  ...
  @botCommand("webhook list", {
    help: "Show webhooks currently configured.",
    category: GenericHookConnection.ServiceCategory,
  })
  public async onWebhookList() {
    const webhooks: GenericHookConnectionState[] = await this.client
      .getRoomState(this.roomId)
      .catch((err: any) => {
        if (err.body.errcode === "M_NOT_FOUND") {
          return []; // not an error to us
    ...
    if (webhooks.length === 0) {
      return this.client.sendHtmlNotice(
        this.roomId,
        md.renderInline("No webhooks configured"),
      );
    } else {
      const feedDescriptions = webhooks
  ...
  @botCommand("webhook remove", {
    help: "Remove a webhook from the room.",
    requiredArgs: ["name"],
    includeUserId: true,
    category: GenericHookConnection.ServiceCategory,
  })
  public async onWebhookRemove(userId: string, name: string) {
    ...
    if (!event || event.disabled === true || Object.keys(event).length === 0) {
      throw new CommandError(
        ...
        `No webhook by the name of "${name}" is configured.`,
      );
    }
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Removed webhook \`${name}\``),
    );
  }

  @botCommand("outbound-hook", {
    help: "Create an outbound webhook.",
  ...
  public async onOutboundHook(userId: string, name: string, url: string) {
    ...
    return this.client.sendNotice(
      this.roomId,
      `Room configured to bridge outbound webhooks. See admin room for the secret token.`,
    );
  }

  @botCommand("figma file", {
    help: "Bridge a Figma file to the room.",
    requiredArgs: ["url"],
    includeUserId: true,
    category: FigmaFileConnection.ServiceCategory,
  })
  public async onFigma(userId: string, url: string) {
    if (!this.config.figma) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support Figma.",
      );
    }

    await this.checkUserPermissions(
      userId,
      FigmaFileConnection.ServiceCategory,
    ...
    if (!res) {
      throw new CommandError(
        "Invalid Figma url",
        "The Figma file url you entered was not valid. It should be in the format of `https://figma.com/file/FILEID/...`.",
      );
    }
    const [, fileId] = res;
    const { connection } = await FigmaFileConnection.provisionConnection(
      this.roomId,
      userId,
      { fileId },
      this.provisionOpts,
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Room configured to bridge Figma file.`),
    );
  }
  ...
  public async onFeed(userId: string, url: string, label?: string) {
    if (!this.config.feeds?.enabled) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support feeds.",
      );
    }
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        throw new CommandError(
          "Invalid URL",
          `${url} doesn't look like a valid feed URL`,
        );
      }
    }
  ...
  @botCommand("feed list", {
    help: "Show feeds currently subscribed to. Supported formats `json` and `yaml`.",
    optionalArgs: ["format"],
    category: FeedConnection.ServiceCategory,
  })
  public async onFeedList(format?: string) {
    ...
    const feeds: FeedConnectionState[] = await this.client
      .getRoomState(this.roomId)
      .catch((err: any) => {
        if (err.body.errcode === "M_NOT_FOUND") {
          return []; // not an error to us
    ...
    if (feeds.length === 0) {
      return this.client.sendHtmlNotice(
        this.roomId,
        md.renderInline("Not subscribed to any feeds"),
      );
    } else {
      const feedDescriptions = feeds
  ...
  public async onFeedRemove(userId: string, url: string) {
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Unsubscribed from \`${url}\``),
    );
  }

  @botCommand("challenghound add", {
  ...
  public async onChallengeHoundAdd(userId: string, url: string) {
    if (!this.config.challengeHound) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support challengeHound.",
      );
    }

    await this.checkUserPermissions(
      userId,
      HoundConnection.ServiceCategory,
      HoundConnection.CanonicalEventType,
    );
    const { connection, challengeName } =
      await HoundConnection.provisionConnection(
        this.roomId,
        userId,
        { url },
        this.provisionOpts,
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Room configured to bridge ${challengeName}. Good luck!`),
    );
  }

  @botCommand("challenghound remove", {
    help: "Unbridge a ChallengeHound challenge.",
    requiredArgs: ["urlOrId"],
    includeUserId: true,
    category: HoundConnection.ServiceCategory,
  })
  public async onChallengeHoundRemove(userId: string, urlOrId: string) {
    ...
    const id = urlOrId.startsWith("http")
      ? HoundConnection.getIdFromURL(urlOrId)
      : urlOrId;
    const event = await this.client
      .getRoomStateEvent(this.roomId, HoundConnection.CanonicalEventType, id)
      .catch((err: any) => {
        if (err.body.errcode === "M_NOT_FOUND") {
          return null; // not an error to us
        }
    ...
    if (!event || Object.keys(event).length === 0) {
      throw new CommandError(
        "Invalid feed URL",
        `Challenge "${id}" is not currently bridged to this room`,
      );
    }
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Unsubscribed from challenge`),
    );
  }
  ...
  private getOpenProjectSafeUrl(urlStr: string): string {
    ...
    if (!projectId) {
      throw new CommandError(
        "Invalid OpenProject",
        "The project url you entered was not valid. It should be in the format of `https://op-instance/projects/PROJECTID/...`",
      );
    }
  ...
  public async onOpenProjectAdd(userId: string, url: string) {
    if (!this.config.openProject) {
      throw new CommandError(
        "not-configured",
        "The bridge is not configured to support open project.",
      );
    }
    ...
    const { connection } = await OpenProjectConnection.provisionConnection(
      this.roomId,
      userId,
      { url: safeUrl },
      this.provisionOpts,
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Room configured to bridge this project`),
    );
  }
  ...
  @botCommand("openproject remove", {
    ...
    requiredArgs: ["url"],
    includeUserId: true,
    category: OpenProjectConnection.ServiceCategory,
  })
  public async onOpenProjectRemove(userId: string, urlStr: string) {
    ...
    if (!event || Object.keys(event).length === 0) {
      throw new CommandError(
        "Invalid OpenProject project URL",
        `OpenProject "${urlStr}" is not currently bridged to this room`,
      );
    }
    ...
    return this.client.sendHtmlNotice(
      this.roomId,
      md.renderInline(`Room no longer bridged to project \`${safeUrl}\`.`),
    );
  }
  ...
  public async onSetupWidget() {
    if (this.config.widgets?.roomSetupWidget === undefined) {
      throw new CommandError(
        "Not configured",
        "The bridge is not configured to support setup widgets",
      );
    }
    if (
  ...
  private async checkUserPermissions(
    ...
    if (
      ...
      throw new CommandError(
        `${userId} does not have permission to manageConnections for ${service}`,
        `You are not permitted to provision connections for ${service}.`,
      );
    }
    if (
      !(await this.client.userHasPowerLevelFor(userId, this.roomId, "", true))
    ) {
      throw new CommandError(
        "not-configured",
        "You must be able to set state in a room ('Change settings') in order to set up new integrations.",
      );
    }
    if (
      !(await this.client.userHasPowerLevelFor(
        this.intent.userId,
        this.roomId,
        stateEventType,
        true,
      ...
      throw new CommandError(
        "Bot lacks power level to set room state",
        "I do not have permission to set up a bridge in this room. Please promote me to an Admin/Moderator.",
      );
    }
  }
}
