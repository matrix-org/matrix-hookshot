...
export class HoundConnection extends BaseConnection implements IConnection {
  static readonly CanonicalEventType =
    "uk.half-shot.matrix-hookshot.challengehound.activity";
  ...
  public static validateState(
    ...
    if (
      ...
      !/^\w{8}(?:-\w{4}){3}-\w{12}$/.test(data.challengeId)
    ) {
  ...
  public static createConnectionForState(
    ...
    { config, intent, storage }: InstantiateConnectionOpts,
  ) {
    if (!config.challengeHound) {
      throw Error("Challenge hound is not configured");
    }
    return new HoundConnection(
      roomId,
      event.stateKey,
      this.validateState(event.content),
      intent,
      storage,
    );
  }
  ...
  static async provisionConnection(
    ...
    _userId: string,
    data: Record<string, unknown> = {},
    { intent, config, storage }: ProvisionConnectionOpts,
  ) {
    if (!config.challengeHound) {
      throw Error("Challenge hound is not configured");
    }
    ...
    const statusDataRequest = await fetch(
      `https://api.challengehound.com/challenges/${validState.challengeId}/status`,
    );
    if (!statusDataRequest.ok) {
      throw new CommandError(
        `Fetch failed, status ${statusDataRequest.status}`,
        "Challenge could not be found. Is it active?",
      );
    }
    const { challengeName } = (await statusDataRequest.json()) as {
      challengeName: string;
    };
    const connection = new HoundConnection(
      roomId,
      validState.challengeId,
      validState,
      intent,
      storage,
    );
  ...
  public async handleNewActivity(activity: HoundActivity) {
    ...
    const existingActivityEventId = await this.storage.getHoundActivity(
      this.challengeId,
      activity.activityId,
    );
    ...
    content["uk.half-shot.matrix-challenger.activity.distance"] = Math.round(
      distance * 1000,
    );
    content["uk.half-shot.matrix-challenger.activity.elevation"] = Math.round(
      parseFloat(activity.elevationMeters),
    );
    content["uk.half-shot.matrix-challenger.activity.duration"] = Math.round(
    ...
    if (existingActivityEventId) {
      log.debug(
        `Updating existing activity ${activity.activityId} ${existingActivityEventId}`,
      );
      content = {
        body: `* ${content.body}`,
        msgtype: "m.notice",
    ...
    const eventId = await this.intent.underlyingClient.sendMessage(
      this.roomId,
      content,
    );
    await this.storage.storeHoundActivityEvent(
      this.challengeId,
      activity.activityId,
      eventId,
    );
