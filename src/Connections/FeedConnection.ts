...
export class FeedConnection extends BaseConnection implements IConnection {
  ...
  public static createConnectionForState(
    ...
    { config, intent }: InstantiateConnectionOpts,
  ) {
  ...
  static validateState(
    ...
    if (typeof data.template !== "undefined") {
      ...
      if (data.template.length > MAX_TEMPLATE_LENGTH) {
        throw new ApiError(
          `Template should not be longer than ${MAX_TEMPLATE_LENGTH} characters`,
          ErrCode.BadValue,
        );
      }
    }
    ...
    if (
      typeof data.notifyOnFailure !== "undefined" &&
      typeof data.notifyOnFailure !== "boolean"
    ) {
  ...
  static async provisionConnection(
    ...
    _userId: string,
    data: Record<string, unknown> = {},
    { intent, config }: ProvisionConnectionOpts,
  ) {
    if (!config.feeds?.enabled) {
      throw new ApiError(
        "RSS/Atom feeds are not configured",
  ...
  constructor(
    ...
    log.info(
      `Connection ${this.connectionId} created for ${roomId}, ${JSON.stringify(state)}`,
    );
  }
  ...
  public isInterestedInStateEvent(
    ...
    return (
      !!FeedConnection.EventTypes.find((e) => e === eventType) &&
      stateKey === this.feedUrl
    );
  }
  ...
  public async provisionerUpdateConfig(
    ...
    if (validatedConfig.url !== this.feedUrl) {
      throw new ApiError(
        "Cannot alter url of existing feed. Please create a new one.",
        ErrCode.BadValue,
      );
    }
