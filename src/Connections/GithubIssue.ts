  ...
  implements IConnection
{
  ...
  public isInterestedInStateEvent(eventType: string, stateKey: string) {
    return (
      GitHubIssueConnection.EventTypes.includes(eventType) &&
  ...
  private async onCommentCreated(
    ...
    const matrixEvent =
      await this.commentProcessor.getEventBodyForGitHubComment(
        comment,
        event.repository,
        event.issue,
      );
  ...
  public async syncIssueState() {
    ...
    const issue = await this.github
      .getOctokitForRepo(this.org, this.repo)
      .issues.get({
    ...
    if (this.state.state !== issue.data.state) {
      if (issue.data.state === "closed") {
        // TODO: Fix
        const closedUserId = this.as.getUserIdForSuffix(
          issue.data.closed_by?.login as string,
        );
        await ensureUserIsInRoom(
