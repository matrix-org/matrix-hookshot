...
export type GitLabRepoConnectionTarget =
  | GitLabRepoConnectionInstanceTarget
...
export type GitLabRepoResponseItem =
  GetConnectionsResponseItem<GitLabRepoConnectionState>;
  ...
  implements IConnection
{
  ...
  static validateState(
    ...
    if (validator(state)) {
      ...
      if (
        !isExistingState &&
        state.enableHooks &&
        !state.enableHooks.every((h) => AllowedEvents.includes(h))
      ) {
        throw new ApiError(
          "`enableHooks` must only contain allowed values",
          ErrCode.BadValue,
        );
      }
  ...
  static async createConnectionForState(
    ...
    { as, intent, storage, tokenStore, config }: InstantiateConnectionOpts,
  ) {
    if (!config.gitlab) {
  ...
  public static async assertUserHasAccessToProject(
    ...
    if (!instance) {
      throw Error(
        `provisionConnection provided an instanceName of ${instanceName} but the instance does not exist`,
      );
    }
    const client = await tokenStore.getGitLabForUser(requester, instance.url);
    if (!client) {
      throw new ApiError(
        "User is not authenticated with GitLab",
    ...
    try {
      ...
      throw new ApiError(
        "Could not determine if the user has access to this project, does the project exist?",
        ErrCode.ForbiddenUser,
      );
    }
    ...
    if (permissionLevel < AccessLevel.Developer) {
      throw new ApiError(
        "You must at least have developer access to bridge this project",
        ErrCode.ForbiddenUser,
      );
    }
  ...
  public static async provisionConnection(
    ...
    if (!instance) {
      throw Error(
        `provisionConnection provided an instanceName of ${validData.instance} but the instance does not exist`,
      );
    }
    const permissionLevel = await this.assertUserHasAccessToProject(
      validData.instance,
      validData.path,
      requester,
      tokenStore,
    ...
    const existing = existingConnections.find(
      (c) =>
        c.roomId === roomId &&
        c.instance.url === connection.instance.url &&
        c.path === connection.path,
    );

    if (existing) {
      throw new ApiError(
        "A GitLab repo connection for this project already exists",
    ...
    if (
      gitlabConfig.webhook.publicUrl &&
      permissionLevel >= AccessLevel.Maintainer
    ) {
      const hooks = await client.projects.hooks.list(project.id);
      const hasHook = hooks.find(
        (h) => h.url === gitlabConfig.webhook.publicUrl,
      );
      if (!hasHook) {
      ...
      warning = {
        ...
        message:
          "You have insufficient permissions on this project to provision a webhook for it. Ask a Maintainer or Owner of the project to add the webhook for you.",
      };
      log.warn(
        `Not creating webhook, permission level is insufficient (${permissionLevel} < ${AccessLevel.Maintainer})`,
      );
    }
    await new GitLabGrantChecker(as, gitlabConfig, tokenStore).grantConnection(
      roomId,
      { instance: validData.instance, path: validData.path },
    );
  ...
  public static async getBase64Avatar(
    avatarUrl: string,
    client: GitLabClient,
  ...
  public static async getConnectionTargets(
    userId: string,
    config: BridgeConfigGitLab,
    filters: GitLabTargetFilter = {},
    tokenStore: UserTokenStore,
    storage: IBridgeStorageProvider,
  ): Promise<GitLabRepoConnectionTarget[]> {
    ...
    const client =
      instanceUrl && (await tokenStore.getGitLabForUser(userId, instanceUrl));
    if (!client) {
      throw new ApiError(
        "Instance is not known or you do not have access to it.",
        ErrCode.NotFound,
      );
    }
    ...
    return (await Promise.all(
      allProjects.map(async (p) => ({
        ...
        avatar_url:
          p.avatar_url &&
          (await this.getBase64Avatar(p.avatar_url, client, storage)),
        description: p.description,
      })),
    )) as GitLabRepoConnectionProjectTarget[];
  }

  private readonly debounceMRComments = new Map<
    string,
    {
      commentCount: number;
      commentNotes?: string[];
  ...
  private readonly discussionThreads = new QuickLRU<
    string,
  ...
  public isInterestedInStateEvent(eventType: string, stateKey: string) {
    return (
      GitLabRepoConnection.EventTypes.includes(eventType) &&
  ...
  @botCommand(
    "create-confidential",
    "Create a confidental issue for this repo",
    ["title"],
    ["description", "labels"],
    true,
  )
  public async onCreateConfidentialIssue(
    userId: string,
    title: string,
    description?: string,
    labels?: string,
  ...
  public async onMergeRequestOpened(event: IGitLabWebhookMREvent) {
    if (
      this.hookFilter.shouldSkip("merge_request", "merge_request.open") ||
      !this.matchesLabelFilter(event)
    ) {
      return;
    }
    log.info(
      `onMergeRequestOpened ${this.roomId} ${this.path} !${event.object_attributes.iid}`,
    );
  ...
  public async onMergeRequestReopened(event: IGitLabWebhookMREvent) {
    if (
      this.hookFilter.shouldSkip("merge_request", "merge_request.reopen") ||
      !this.matchesLabelFilter(event)
    ) {
      return;
    }
    log.info(
      `onMergeRequestReopened ${this.roomId} ${this.path} !${event.object_attributes.iid}`,
    );
    this.validateMREvent(event);
    const orgRepoName = event.project.path_with_namespace;
  ...
  public async onMergeRequestClosed(event: IGitLabWebhookMREvent) {
    if (
      this.hookFilter.shouldSkip("merge_request", "merge_request.close") ||
      !this.matchesLabelFilter(event)
    ) {
      return;
    }
    log.info(
      `onMergeRequestClosed ${this.roomId} ${this.path} !${event.object_attributes.iid}`,
    );
    this.validateMREvent(event);
    const orgRepoName = event.project.path_with_namespace;
  ...
  public async onMergeRequestMerged(event: IGitLabWebhookMREvent) {
    if (
      this.hookFilter.shouldSkip("merge_request", "merge_request.merge") ||
      !this.matchesLabelFilter(event)
    ) {
      return;
    }
    log.info(
      `onMergeRequestMerged ${this.roomId} ${this.path} !${event.object_attributes.iid}`,
    );
    this.validateMREvent(event);
    const orgRepoName = event.project.path_with_namespace;
  ...
  public async onMergeRequestUpdate(event: IGitLabWebhookMREvent) {
    ...
    log.info(
      `onMergeRequestUpdate ${this.roomId} ${this.instance}/${this.path} !${event.object_attributes.iid}`,
    );
    this.validateMREvent(event);
  ...
  public async onGitLabTagPush(event: IGitLabWebhookTagPushEvent) {
    ...
    log.info(
      `onGitLabTagPush ${this.roomId} ${this.instance.url}/${this.path} ${event.ref}`,
    );
...
${data.description}`;
  ...
  public async onPipelineTriggered(event: IGitLabWebhookPipelineEvent) {
    if (this.hookFilter.shouldSkip("pipeline", "pipeline.running")) {
      return;
    }
  ...
  public async onPipelineFailed(event: IGitLabWebhookPipelineEvent) {
    if (this.hookFilter.shouldSkip("pipeline", "pipeline.failed")) {
      return;
    }

    log.info(
      `onPipelineFailed ${this.roomId} ${this.instance.url}/${this.path}`,
    );
    const { ref, duration } = event.object_attributes;
  ...
  public async onPipelineCanceled(event: IGitLabWebhookPipelineEvent) {
    if (this.hookFilter.shouldSkip("pipeline", "pipeline.canceled")) {
      return;
    }

    log.info(
      `onPipelineCanceled ${this.roomId} ${this.instance.url}/${this.path}`,
    );
    const { ref, duration } = event.object_attributes;
  ...
  private async renderDebouncedMergeRequest(
    ...
    let relation;
    const discussionWithThread = result.discussions.find((discussionId) =>
      this.discussionThreads.has(discussionId),
    );
    if (discussionWithThread) {
      const threadEventId = await this.discussionThreads
        .get(discussionWithThread)
        ?.catch(() => {
          /* already logged */
        });
      if (threadEventId) {
    ...
    void this.persistDiscussionThreads().catch((ex) => {
      log.error(
        `Failed to persistently store Gitlab discussion threads for connection ${this.connectionId}:`,
        ex,
      );
    });
  }
  ...
  private debounceMergeRequestReview(
    ...
    if (existing) {
      ...
      existing.timeout = setTimeout(
        () => this.renderDebouncedMergeRequest(uniqueId, mergeRequest, project),
        this.commentDebounceMs,
      );
      return;
    }
  ...
  public async onMergeRequestReviewed(event: IGitLabWebhookMREvent) {
    if (
      this.hookFilter.shouldSkip(
        "merge_request",
        "merge_request.review",
        `merge_request.${event.object_attributes.action}`,
      ) ||
      !this.matchesLabelFilter(event)
    ) {
      return;
  ...
  public async onMergeRequestCommentCreated(event: IGitLabWebhookNoteEvent) {
    ...
    if (
      this.hookFilter.shouldSkip("merge_request", "merge_request.review") ||
      !this.matchesLabelFilter(event.merge_request)
    ) {
      return;
    }
    log.info(
    ...
    this.debounceMergeRequestReview(
      ...
      {
        commentCount: 1,
        commentNotes: this.state.includeCommentBody
  ...
  public matchesLabelFilter(itemWithLabels: {
    labels?: { title: string }[];
  }): boolean {
  ...
  private setDiscussionThreads(
    discussionThreads: SerializedGitlabDiscussionThreads,
  ): void {
  ...
  private async persistDiscussionThreads(): Promise<void> {
    ...
    for (const [
      discussionId,
      eventIdPromise,
    ] of this.discussionThreads.entriesAscending()) {
      const eventId = await eventIdPromise.catch(() => {
        /* logged elsewhere */
      });
      if (eventId) {
    ...
    return this.storage.setGitlabDiscussionThreads(
      this.connectionId,
      serialized,
    );
  }
