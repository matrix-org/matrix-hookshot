...
export class FigmaWebhooksRouter {
  ...
  private onWebhook(
    req: Request<unknown, unknown, FigmaPayload, unknown>,
    res: Response<string | { error: string }>,
  ) {
    const payload = req.body;
    const instance = Object.entries(this.config.instances).find(
      ([, p]) => p.passcode === payload.passcode,
    );
    ...
    if (
      ...
      res
        .status(400)
        .send({ error: "Missing required object keys file_name, file_key" });
      return;
    }
    res.status(200).send();
    this.queue.push({
