...
export class FeedError extends Error {
  ...
  get shouldErrorBeSilent() {
    ...
    if (
      axios.isAxiosError(this.cause) &&
      this.cause.code &&
      ["ECONNABORTED", "ECONNRESET"].includes(this.cause.code)
    ) {
...
export class FeedReader {
  ...
  private cacheTimes: Map<string, { etag?: string; lastModified?: string }> =
    new Map();
  ...
  static readonly seenEntriesEventType =
    "uk.half-shot.matrix-hookshot.feed.reader.seenEntries";
  ...
  constructor(
    ...
    this.connections =
      this.connectionManager.getAllConnectionsOfType(FeedConnection);
    ...
    connectionManager.on("connection-removed", (removed) => {
      ...
      this.connections = this.connections.filter((c) => {
        ...
        if (c.connectionId !== removed.connectionId) {
          shouldKeepUrl =
            shouldKeepUrl || normalizeUrl(c.feedUrl) === normalisedUrl;
          return true;
        }
        return false;
      });
      if (shouldKeepUrl) {
        log.info(
          `Connection removed, but not removing "${normalisedUrl}" as it is still in use`,
        );
        return;
      }
  ...
  private calculateInitialFeedUrls(): Set<string> {
    ...
    for (const conn of this.connections) {
      try {
        ...
        log.error(
          `Invalid feedUrl for connection ${conn.connectionId}: ${conn.feedUrl}. It will not be tracked`,
          err,
        );
      }
    }
  ...
  public async pollFeed(url: string): Promise<boolean> {
    ...
    try {
      ...
      if (feed) {
        ...
        const seenItems = await this.storage.hasSeenFeedGuids(
          url,
          ...feed.items
        ...
        for (const item of feed.items) {
          ...
          if (initialSync) {
            log.debug(
              `Skipping entry ${item.id ?? item.hashId} since we're performing an initial sync`,
            );
            continue;
          }
          const entry = {
            feed: {
              title: isNonEmptyString(feed.title)
                ? stripHtml(feed.title)
                : null,
              url: url,
            },
          ...
          this.queue.push<FeedEntry>({
            eventName: "feed.entry",
            sender: "FeedReader",
            data: entry,
          });
        }
      ...
      this.queue.push<FeedSuccess>({
        eventName: "feed.success",
        sender: "FeedReader",
        data: { url },
      });
      ...
      log.error(
        "Unable to read feed:",
        feedError.message,
        `backing off for ${backoffDuration}ms`,
      );
      this.queue.push<FeedError>({
  ...
  public async pollFeeds(workerId: number): Promise<void> {
    ...
    Metrics.feedsFailing.set(
      { reason: "parsing" },
      this.feedsFailingParsing.size,
    );
    Metrics.feedsFailingDeprecated.set(
      { reason: "http" },
      this.feedsFailingHttp.size,
    );
    Metrics.feedsFailingDeprecated.set(
      { reason: "parsing" },
      this.feedsFailingParsing.size,
    ...
    log.debug(
      `Checking for updates in ${this.feedQueue.length()} RSS/Atom feeds (worker: ${workerId})`,
    );
    ...
    if (url) {
      ...
      log.debug(
        `Feed fetching took ${elapsed / 1000}s, sleeping for ${sleepFor / 1000}s`,
      );

      if (elapsed > this.sleepingInterval) {
        log.warn(
          `It took us longer to update the feeds than the configured pool interval`,
        );
      }
    } else {
