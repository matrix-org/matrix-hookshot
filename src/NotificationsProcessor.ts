...
export class NotificationProcessor {
  ...
  private static formatNotification(
    notif: GitHubUserNotification,
    diff: IssueDiff | null,
    newComment: boolean,
  ) {
    const user = diff ? ` by ${this.formatUser(diff?.user)}` : "";
    let plain = `${this.getEmojiForNotifType(notif)} [${notif.subject.title}](${notif.subject.url_data?.html_url})${user}`;
    ...
    if (diff) {
      ...
      if (diff.merged) {
        ...
        const state =
          diff.state[0].toUpperCase() + diff.state.slice(1).toLowerCase();
    ...
    if (newComment) {
      const comment = notif.subject
        .latest_comment_url_data as IssuesGetCommentResponseData;
  ...
  constructor(
    private storage: IBridgeStorageProvider,
    private matrixSender: MessageSenderClient,
  ) {}
  ...
  public async onUserEvents(msg: UserNotificationsEvent, adminRoom: AdminRoom) {
    ...
    for (const event of msg.events) {
      const isIssueOrPR =
        event.subject.type === "Issue" || event.subject.type === "PullRequest";
      try {
        await this.handleUserNotification(
          msg.roomId,
          event,
          adminRoom.notifFilter,
        );
  ...
  private diffIssueChanges(curr: PROrIssue, prev: PROrIssue): IssueDiff {
    ...
    let mergedBy = null;
    if (
      (curr as PullGetResponseData).merged !==
      (prev as PullGetResponseData).merged
    ) {
      merged = true;
    ...
    const diff: IssueDiff = {
      ...
      mergedBy,
      assignee:
        curr.assignee?.id === prev.assignee?.id ? null : [curr.assignee],
  ...
  private async formatIssueOrPullRequest(
    roomId: string,
    notif: GitHubUserNotification,
  ) {
    const issueNumber = notif.subject.url_data?.number.toString();
    let diff = null;
    if (issueNumber) {
      const prevIssue: IssuesGetResponseData | null =
        await this.storage.getGithubIssue(
          notif.repository.full_name,
          issueNumber,
          roomId,
        );
    ...
    const newComment =
      ...
      !!issueNumber &&
      notif.subject.latest_comment_url !==
        (await this.storage.getLastNotifCommentUrl(
          notif.repository.full_name,
          issueNumber,
          roomId,
        ));

    const formatted = NotificationProcessor.formatNotification(
      notif,
      diff,
      newComment,
    );
  ...
  private async handleUserNotification(
    ...
    if (notif.reason === "security_alert") {
      return this.matrixSender.sendMatrixMessage(
        roomId,
        this.formatSecurityAlert(notif),
      );
    } else if (
    ...
    const genericNotif = NotificationProcessor.formatNotification(
      notif,
      null,
      false,
    );
