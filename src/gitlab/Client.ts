...
export class GitLabClient {
  ...
  public static splitUrlIntoParts(
    ...
    for (const [instanceKey, instanceConfig] of Object.entries(instances)) {
      if (url.startsWith(instanceConfig.url)) {
        return [
          instanceKey,
          url
            .substr(instanceConfig.url.length)
            .split("/")
  ...
  private async createIssue(
    ...
    return (
      await axios.post(
        `api/v4/projects/${encodeURIComponent(opts.id)}/issues`,
        opts,
        this.defaultConfig,
      )
    ).data;
  ...
  private async getIssue(opts: GetIssueOpts): Promise<GetIssueResponse> {
    try {
      return (
        await axios.get(
          `api/v4/projects/${opts.projects.join("%2F")}/issues/${opts.issue}`,
          this.defaultConfig,
        )
      ).data;
    } catch (ex) {
  ...
  private async editIssue(opts: EditIssueOpts): Promise<CreateIssueResponse> {
    return (
      await axios.put(
        `api/v4/projects/${encodeURIComponent(opts.id)}/issues/${opts.issue_iid}`,
        opts,
        this.defaultConfig,
      )
    ).data;
  ...
  private async getProject(id: ProjectId): Promise<GetProjectResponse> {
    try {
      return (
        await axios.get(
          `api/v4/projects/${encodeURIComponent(id)}`,
          this.defaultConfig,
        )
      ).data;
    } catch (ex) {
  ...
  private async listProjects(
    ...
    try {
      const path = inGroup
        ? `api/v4/groups/${encodeURIComponent(inGroup)}/projects`
        : "api/v4/projects";
      return (
  ...
  private async getProjectHooks(id: ProjectId): Promise<ProjectHook[]> {
    try {
      return (
        await axios.get(
          `api/v4/projects/${encodeURIComponent(id)}/hooks`,
          this.defaultConfig,
        )
      ).data;
    } catch (ex) {
  ...
  private async addProjectHook(
    id: ProjectId,
    opts: ProjectHookOpts,
  ): Promise<ProjectHook> {
    try {
      return (
        await axios.post(
  ...
  public async getEvents(opts: EventsOpts) {
    const after =
      `${opts.after.getFullYear()}-` +
      `${(opts.after.getMonth() + 1).toString().padStart(2, "0")}` +
      `-${opts.after.getDay().toString().padStart(2, "0")}`;
    return (await axios.get(`api/v4/events?after=${after}`, this.defaultConfig))
  ...
  public async createIssueNote(
    projectParts: string[],
    issueId: number,
    opts: CreateIssueNoteOpts,
  ): Promise<CreateIssueNoteResponse> {
    try {
      ...
      return (
        await axios.post(
          `api/v4/projects/${projectParts.join("%2F")}/issues/${issueId}/notes?${qp}`,
          undefined,
          this.defaultConfig,
        )
      ).data as CreateIssueNoteResponse;
    } catch (ex) {
  ...
  public async getProjectAccessLevel(id: ProjectId): Promise<AccessLevel> {
    try {
      ...
      const { data } = await axios.get(
        `api/v4/projects/${encodeURIComponent(id)}/members/all/${me.id}`,
        this.defaultConfig,
      );
      if (typeof data?.access_level !== "number") {
        throw Error(
          `Unexpected value for data.access_level. '${data?.access_level}'`,
        );
      }
