...
export class GitHubOAuthError extends Error {
  constructor(errorResponse: GitHubOAuthErrorResponse) {
    super(
      `OAuth interaction failed with ${errorResponse.error}: ${errorResponse.error_description}. See ${errorResponse.error_uri}`,
    );
  }
}

export function getNameForGitHubAccount(
  account: { login: string } | { name?: string },
) {
  return "login" in account
    ? account.login
    : (account.name ?? NAMELESS_ORG_PLACEHOLDER);
}
...
export class GithubInstance {
  ...
  public static baseOctokitConfig(baseUrl: URL) {
    ...
    const url = (
      baseUrl.hostname === GITHUB_CLOUD_URL.hostname
        ? baseUrl
        : new URL("/api/v3", baseUrl)
    ).toString();
    return {
      userAgent: UserAgent,
  ...
  public static async refreshAccessToken(
    ...
    const response: Record<string, unknown> = Object.fromEntries(
      new URLSearchParams(accessTokenRes.data),
    );
    if ("error" in response) {
      throw new GitHubOAuthError(
        response as unknown as GitHubOAuthErrorResponse,
      );
    }
  ...
  public getSafeOctokitForRepo(orgName: string, repoName?: string) {
    const targetName = (
      repoName ? `${orgName}/${repoName}` : orgName
    ).toLowerCase();
    for (const install of this.installationsCache.values()) {
      if (
        install.matchesRepository.includes(targetName) ||
        install.matchesRepository.includes(`${targetName.split("/")[0]}/*`)
      ) {
  ...
  public async start() {
    ...
    if (!appDetails.data) {
      throw Error(
        "No information returned about GitHub App. Is your GitHub App configured correctly?",
      );
    }
    ...
    let page = 1;
    do {
      const installations = await this.internalOctokit.apps.listInstallations({
        per_page: 100,
        page: page++,
      });
      for (const install of installations.data) {
        if (install.suspended_at) {
          log.warn(
            `GitHub app install ${install.id} was suspended. GitHub connections using this install may not work correctly`,
          );
          continue;
        }
        try {
  ...
  private async addInstallation(
    install: InstallationDataType,
    ...
    if (
      ...
      const repos = await installOctokit.apps.listReposAccessibleToInstallation(
        { per_page: 100 },
      );
      matchesRepository.push(
        ...repos.data.repositories.map((r) => r.full_name.toLowerCase()),
      );
    }
  ...
  public onInstallationCreated(
    data:
      | GitHubWebhookTypes.InstallationCreatedEvent
      | GitHubWebhookTypes.InstallationUnsuspendEvent,
  ) {
    this.addInstallation(
      data.installation as InstallationDataType,
      data.repositories,
    );
  }
  ...
  public onInstallationRemoved(
    data:
      | GitHubWebhookTypes.InstallationDeletedEvent
      | GitHubWebhookTypes.InstallationSuspendEvent,
  ) {
  ...
  public static generateOAuthUrl(
    baseUrl: URL,
    action: "authorize" | "access_token",
    params: OAuthUrlParameters,
  ) {
...
export class GithubGraphQLClient {
  ...
  public async getDiscussionByNumber(
    owner: string,
    name: string,
    number: number,
  ) {
