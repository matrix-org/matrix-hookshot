...
export class GitHubBotCommands extends AdminRoomCommandHandler {
  ...
  public async loginCommand() {
    ...
    if (!this.config.github.oauth) {
      throw new CommandError(
        "no-github-support",
        "The bridge is not configured with GitHub OAuth support.",
      );
    }
  ...
  public async setGHPersonalAccessToken(accessToken: string) {
    ...
    let me;
    try {
      const octokit = GithubInstance.createUserOctokit(
        accessToken,
        this.config.github.baseUrl,
      );
      ...
      await this.sendNotice(
        "Could not authenticate with GitHub. Is your token correct?",
      );
      return;
    }
  ...
  public async getTokenStatus() {
    ...
    try {
      ...
      if (ex instanceof TokenError && ex.code === TokenErrorCode.EXPIRED) {
        await this.sendNotice(
          "Your authentication is no longer valid, please login again.",
        );
      } else {
        // Generic catch-all.
        await this.sendNotice(
          "The bridge was unable to authenticate as you, please login again.",
        );
      }
    }
  }
