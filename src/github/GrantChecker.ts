...
export class GitHubGrantChecker extends GrantChecker<GitHubGrantConnectionId> {
  ...
  protected async checkFallback(
    roomId: string,
    connectionId: GitHubGrantConnectionId,
    sender?: string,
  ) {
    if (!sender) {
    ...
    try {
      await GitHubRepoConnection.assertUserHasAccessToRepo(
        sender,
        connectionId.org,
        connectionId.repo,
        this.tokenStore,
      ...
      log.info(
        `Tried to check fallback for ${roomId}: ${sender} does not have access to ${connectionId.org}/${connectionId.repo}`,
        ex,
      );
      return false;
    }
