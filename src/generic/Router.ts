...
export class GenericWebhooksRouter {
  ...
  private onWebhook(
    req: Request<{ hookId: string }, unknown, unknown, unknown>,
    res: Response<unknown | { ok: false; error: string }>,
    next: NextFunction,
  ) {
    if (req.method === "GET" && !this.allowGet) {
      throw new ApiError(
        "Invalid Method. Expecting PUT or POST",
        ErrCode.MethodNotAllowed,
      );
    }

    if (!["PUT", "GET", "POST"].includes(req.method)) {
      throw new ApiError(
        "Invalid Method. Expecting PUT, GET or POST",
        ErrCode.MethodNotAllowed,
      );
    }
    ...
    this.queue
      .pushWait<GenericWebhookEvent, GenericWebhookEventResult>(
        {
      ...
      .then((response) => {
        if (response.notFound) {
          ...
          res
            ...
            .send({ ok: false, error: "Webhook not found" });
          ...
          res
            .status(response.response?.statusCode ?? StatusCodes.OK)
            .send(body);
        } else if (response.successful === false) {
          res
            .status(response.statusCode ?? StatusCodes.INTERNAL_SERVER_ERROR)
            .send({
              ok: false,
              error: response.error || "Failed to process webhook",
            });
        } else {
      ...
      .catch((err) => {
        ...
        res
          ...
          .send({ ok: false, error: "Failed to handle webhook" });
      });
  }
