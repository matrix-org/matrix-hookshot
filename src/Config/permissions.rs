...
pub fn permission_level_to_int(level: String) -> napi::Result<u32> {
  match level.as_str() {
    ...
    "login" => Ok(2),
    "notifications" => Ok(3),
    "manage-connections" => Ok(4),
    "admin" => Ok(5),
    _ => {
      Err(napi::Error::new(napi::Status::InvalidArg, "provided level wasn't valid".to_string()))
    }
  }
}

#[napi]
pub fn permissions_check_action(config: Vec<BridgeConfigActorPermission>, mxid: String, service: String, permission: String, target: String) -> napi::Result<bool> {
  ...
  for actor_permission in config.iter() {
    if actor_permission.actor != domain && actor_permission.actor != mxid && actor_permission.actor != "*" {
      continue;
    }
    for actor_service in actor_permission.services.iter() {
      ...
      match &actor_service.targets {
        Some(targets) => {
          if targets.iter().any(|e| *e == target) == false {
            continue;
          }
        }
        None => {}
