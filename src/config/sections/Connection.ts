...
export function validateConnectionConfig(connection: Record<keyof BridgeConfigConnectionConfig, unknown>, enabledServices: ConnectionType[]): connection is BridgeConfigConnectionConfig {
    ...
    const resolvedcType = ConnectionDeclarations.find((c) => c.EventTypes.includes(cType));
    if (!resolvedcType) {
        throw new ConfigError("connectionType", "is not a known connection type");
    }
    if (!resolvedcType.SupportsStaticConfiguration) {
        throw new ConfigError("connectionType", "does not support static configuration");
    }
    if (!enabledServices.includes(resolvedcType.ServiceCategory)) {
        throw new ConfigError("connectionType", `Service '${resolvedcType.ServiceCategory}' is not enabled in the config.`);
    }
