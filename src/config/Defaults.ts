...
function renderSection(
  ...
  parentNode: YAMLSeq | YAML.Document = doc,
  ...
  entries.forEach(([key, value]) => {
    ...
    if (typeof value === "object" && !Array.isArray(value)) {
      ...
      renderSection(
        ...
        newNode as YAMLSeq,
        optional ?? parentIsOptional,
      );
    } else if (typeof value === "function") {
      if (value.length !== 0) {
        throw Error(
          "Only zero-argument functions are allowed as config values",
        );
      }
    ...
    if (optional && !parentIsOptional) {
      ...
      parentNode.comment =
        (parentNode.comment || "") + tempDoc.toString() + `\n`;
      return;
    }
