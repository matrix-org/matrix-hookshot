...
import {
  AdminAccountData,
  AdminRoomCommandHandler,
  Category,
} from "./AdminRoomCommandHandler";
import {
  botCommand,
  compileBotCommands,
  handleCommand,
...
type ProjectsListForUserResponseData =
  Endpoints["GET /users/{username}/projects"]["response"];

const md = new markdown();
const log = new Logger("AdminRoom");

export const LEGACY_BRIDGE_ROOM_TYPE = "uk.half-shot.matrix-github.room";
export const LEGACY_BRIDGE_NOTIF_TYPE =
  "uk.half-shot.matrix-github.notif_state";
export const LEGACY_BRIDGE_GITLAB_NOTIF_TYPE =
  "uk.half-shot.matrix-github.gitlab.notif_state";

export const BRIDGE_ROOM_TYPE = "uk.half-shot.matrix-hookshot.github.room";
export const BRIDGE_NOTIF_TYPE =
  "uk.half-shot.matrix-hookshot.github.notif_state";
export const BRIDGE_GITLAB_NOTIF_TYPE =
  "uk.half-shot.matrix-hookshot.gitlab.notif_state";
export class AdminRoom extends AdminRoomCommandHandler {
  ...
  public async getNotifSince(type: "github" | "gitlab", instanceName?: string) {
    if (type === "gitlab") {
      try {
        let accountData: null | { since: number } =
          await this.botIntent.underlyingClient.getSafeRoomAccountData(
  ...
  public async helpCommand() {
    ...
    return this.botIntent.sendEvent(
      this.roomId,
      AdminRoom.helpMessage(undefined, enabledCategories),
    );
  }

  @botCommand("disconnect", {
    help: "Remove a connection",
    requiredArgs: ["roomId", "id"],
    category: Category.ConnectionManagement,
  })
  public async disconnect(roomId: string, id: string) {
    ...
    try {
      ...
      await this.sendNotice(
        "Connection could not be removed: see debug logs for details",
      );
    }
  }
  ...
  public async setGitHubNotificationsStateToggle() {
    ...
    await this.sendNotice(
      `${newData.github?.notifications?.enabled ? "En" : "Dis"}abled GitHub notifcations`,
    );
  }
  ...
  public async getGitHubNotificationsState() {
    ...
    return this.sendNotice(
      `Notifications are enabled, ${this.notificationsParticipating("github") ? "Showing only events you are particiapting in." : "Showing all events."}`,
    );
  }

  @botCommand("github list-connections", {
    help: "List currently bridged Github rooms",
    category: Category.ConnectionManagement,
  })
  public async listGithubConnections() {
    if (!this.config.github) {
      return this.sendNotice(
        "The bridge is not configured with GitHub support.",
      );
    }
    ...
    const connections = {
      repos:
        this.connectionManager.getAllConnectionsOfType(GitHubRepoConnection),
      issues: this.connectionManager.getAllConnectionsOfType(
        GitHubIssueConnection,
      ),
      discussions: this.connectionManager.getAllConnectionsOfType(
        GitHubDiscussionSpace,
      ),
    };

    const reposFormatted = connections.repos
      .map(
        (c) =>
          ` - ${c.org}/${c.repo} (ID: \`${c.connectionId}\`, Room: \`${c.roomId}\`)`,
      )
      .join("\n");
    const issuesFormatted = connections.issues
      .map(
        (c) =>
          ` - ${c.org}/${c.repo}/${c.issueNumber} (Room: \`${c.roomId}\`, ID: \`${c.connectionId}\`)`,
      )
      .join("\n");
    const discussionsFormatted = connections.discussions
      .map(
        (c) =>
          ` - ${c.owner}/${c.repo} (Room: \`${c.roomId}\`, ID: \`${c.connectionId}\`)`,
      )
      .join("\n");
    ...
    const content =
      [
        ...
        connections.discussions.length > 0
  ...
  private async listGitHubProjectsForUser(username?: string, repo?: string) {
    ...
    try {
      if (repo) {
        res = (
          await octokit.projects.listForRepo({
            repo,
            owner: username,
          })
        ).data;
      ...
      res = (
        await octokit.projects.listForUser({
          username,
        })
      ).data;
    } catch (ex) {
      ...
      return this.sendNotice(
        `Failed to fetch projects due to an error. See logs for details.`,
      );
    }
  ...
  @botCommand("github project list-for-org", {
  ...
  private async openProject(projectId: string) {
    ...
    try {
      ...
      return this.sendNotice(
        `Failed to fetch project due to an error. See logs for details.`,
      );
    }
  }
  ...
  private async listDiscussions(
    ...
    try {
      ...
      const discussions = await graphql.getDiscussionByNumber(
        owner,
        repo,
        number,
      );
      ...
      return this.sendNotice(
        `Failed to fetch discussions due to an error. See logs for details.`,
      );
    }
  }
  ...
  private async gitLabOpenIssue(url: string) {
    if (!this.config.gitlab) {
      return this.sendNotice(
        "The bridge is not configured with GitLab support.",
      );
    }

    const urlResult = GitLabClient.splitUrlIntoParts(
      this.config.gitlab.instances,
      url,
    );
    if (!urlResult) {
      return this.sendNotice(
        "The URL was not understood. The URL must be an issue and the bridge must know of the GitLab instance.",
      );
    }
    const [instanceName, parts] = urlResult;
    const instance = this.config.gitlab.instances[instanceName];
    const client = await this.tokenStore.getGitLabForUser(
      this.userId,
      instance.url,
    );
    if (!client) {
      return this.sendNotice(
        "You have not added a personal access token for GitLab.",
      );
    }
    const getIssueOpts = {
    ...
    log.info(
      `Looking up issue ${instanceName} ${getIssueOpts.projects.join("/")}#${getIssueOpts.issue}`,
    );
  ...
  public async setGitLabPersonalAccessToken(
    ...
    if (!instance) {
      return this.sendNotice(
        "The bridge is not configured for this GitLab instance. Ask your administrator for a list of instances.",
      );
    }
    try {
      ...
      return this.sendNotice(
        "Could not authenticate with GitLab. Is your token correct?",
      );
    }
    await this.sendNotice(`Connected as ${me.username}. Token stored.`);
    return this.tokenStore.storeUserToken(
      "gitlab",
      this.userId,
      accessToken,
      instance.url,
  ...
  public async gitlabHasPersonalToken(instanceName: string) {
    ...
    if (!instance) {
      return this.sendNotice(
        "The bridge is not configured for this GitLab instance.",
      );
    }
    const result = await this.tokenStore.getUserToken(
      "gitlab",
      this.userId,
      instance.url,
    );
  ...
  public async getFilters() {
    ...
    const filterText = Object.entries(this.notifFilter.filters)
      .map(([name, value]) => {
        const userText = value.users.length
          ? `users: ${value.users.join("|")}`
          : "";
        const reposText = value.repos.length
          ? `users: ${value.repos.join("|")}`
          : "";
        const orgsText = value.orgs.length
          ? `users: ${value.orgs.join("|")}`
          : "";
  ...
  public async setFilter(name: string, ...parameters: string[]) {
    const orgs = parameters
      .filter((param) => param.toLowerCase().startsWith("orgs:"))
      .map((param) => param.toLowerCase().substring("orgs:".length).split(","))
      .flat();
    const users = parameters
      .filter((param) => param.toLowerCase().startsWith("users:"))
      .map((param) => param.toLowerCase().substring("users:".length).split(","))
      .flat();
    const repos = parameters
      .filter((param) => param.toLowerCase().startsWith("repos:"))
      .map((param) => param.toLowerCase().substring("repos:".length).split(","))
      .flat();
    if (orgs.length + users.length + repos.length === 0) {
      return this.sendNotice(
        "You must specify some filter options like 'orgs:matrix-org,half-shot', 'users:Half-Shot' or 'repos:matrix-hookshot'.",
      );
    }
    ...
    await this.botIntent.underlyingClient.sendStateEvent(
      ...
      "",
      this.notifFilter.getStateContent(),
    );
    return this.sendNotice(
      `Stored new filter "${name}". You can now apply the filter by saying 'filters notifications toggle $name'.`,
    );
  }
  ...
  private canAdminConnections(service: string): boolean {
    return this.config.checkPermission(
      this.userId,
      service,
      BridgePermissionLevel.admin,
    );
  }

  private async saveAccountData(
    updateFn: (record: AdminAccountData) => AdminAccountData,
  ) {
    let oldData: AdminAccountData | null =
      await this.botIntent.underlyingClient.getSafeRoomAccountData(
        BRIDGE_ROOM_TYPE,
        this.roomId,
        null,
    ...
    if (!oldData) {
      oldData = await this.botIntent.underlyingClient.getSafeRoomAccountData(
    ...
    await this.botIntent.underlyingClient.setRoomAccountData(
      BRIDGE_ROOM_TYPE,
      this.roomId,
      newData,
    );
  ...
  public async handleCommand(eventId: string, command: string) {
    const checkPermission = (service: string, level: BridgePermissionLevel) =>
      this.config.checkPermission(this.userId, service, level);
    ...
    if (!result.handled) {
      return this.sendNotice(
        "Command not understood. For a list of commands, try `help`.",
      );
    }
    ...
    if ("humanError" in result) {
      ...
      return this.sendNotice(
        `Failed to handle command: A unknown failure occurred. Contact your bridge admin`,
      );
    }
    return null;
  }
...
const res = compileBotCommands(
  ...
  JiraBotCommands.prototype as any,
  OpenProjectBotCommands.prototype as any,
);
