...
import { OpenProjectWorkPackageCacheState, workPackageToCacheState } from "./state";
import { OpenProjectWorkPackage } from "./types";


export function formatWorkPackageForMatrix(pkg: OpenProjectWorkPackage, baseURL: URL) {
    const url = new URL(baseURL.href + `projects/${pkg._embedded.project.identifier}/work_packages/${pkg.id}`, baseURL).toString();
    return {
        'org.matrix.matrix-hookshot.openproject.work_package': {
            ...
            author: {
                ...
                url: new URL(baseURL.href + `users/${pkg._embedded.author.id}`, baseURL).toString(),
            },
            assignee: pkg._embedded.assignee && {
                name: pkg._embedded.assignee?.name,
                url: new URL(baseURL.href + `users/${pkg._embedded.assignee?.id}`, baseURL).toString(),
            },
            status: {
        ...
        'org.matrix.matrix-hookshot.openproject.project': {
            ...
            url: new URL(baseURL.href + `projects/${pkg._embedded.project.id}`, baseURL).toString(),
        },
        external_url: url,
    }
}

export function formatWorkPackageDiff(old: OpenProjectWorkPackageCacheState, pkg: OpenProjectWorkPackage): {changes: string[], postfix?: string, eventKind: OpenProjectEventsNames}|null {
    ...
    if (old.percentageDone !== current.percentageDone) {
        if (current.percentageDone) {
            changes.push(`set the work completed percentage to **${current.percentageDone}%**`);
        }
        // No point sending anything about removal.
        eventKind = "work_package:workpercent_changed";
    }
    if (old.priority?.id !== current.priority?.id) {
        changes.push(`changed the priority from **${old.priority?.name}** to **${current.priority?.name ?? "none"}**`);
    ...
    if (old.status.id !== current.status.id) {
        changes.push(`changed the status from **${old.status?.name}** to **${current.status?.name ?? "none"}**`);
    }
