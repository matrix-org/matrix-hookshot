...
export class OpenProjectWidgetAPI {
    public static async getAuth(req: ProvisioningRequest, res: Response<GetAuthResponse>, tokenStore: UserTokenStore) {
        if (!req.userId) {
        ...
        if (user) {
            ...
            if (!tokenStore.openProjectOAuth) {
                throw new ApiError('OAuth is not supported', ErrCode.UnsupportedOperation);
            }
