...
const ACCOUNT_DATA_GITLAB_TYPE =
  "uk.half-shot.matrix-hookshot.gitlab.password-store:";
const ACCOUNT_DATA_JIRA_TYPE =
  "uk.half-shot.matrix-hookshot.jira.password-store:";
const ACCOUNT_DATA_OPENPROJECT_TYPE =
  "uk.half-shot.matrix-hookshot.jira.token-store:";

const LEGACY_ACCOUNT_DATA_TYPE = "uk.half-shot.matrix-github.password-store:";
const LEGACY_ACCOUNT_DATA_GITLAB_TYPE =
  "uk.half-shot.matrix-github.gitlab.password-store:";
...
export class UserTokenStore extends TypedEmitter<Emitter> {
  public static async fromKeyPath(
    keyPath: string,
    intent: Intent,
    config: BridgeConfig,
  ) {
  ...
  private oauthSessionStore: Map<
    string,
    { userId: string; timeout: NodeJS.Timeout }
  > = new Map();
  ...
  constructor(
    ...
    if (config.jira?.oauth) {
      if ("client_id" in config.jira.oauth) {
        ...
        this.jiraOAuth = new JiraOnPremOAuth(
          config.jira.oauth,
          config.jira.url,
        );
      } else {
    ...
    if (config.openProject?.oauth) {
      this.openProjectOAuth = new OpenProjectOAuth(
        this,
        config.openProject.oauth,
  ...
  public async storeUserToken(
    ...
    if (
      !this.config.checkPermission(userId, type, BridgePermissionLevel.login)
    ) {
      throw new ApiError(
        "User does not have permission to log in to service",
  ...
  public async clearUserToken(
    ...
    const key = tokenKey(type, userId, false, instanceUrl);
    const obj = await this.intent.underlyingClient.getSafeAccountData<
      StoredTokenData | DeletedTokenData
    >(key);
  ...
  public async storeOpenProjectToken(
    userId: string,
    token: OpenProjectStoredToken,
  ) {
  ...
  public async getUserToken(
    ...
    try {
      let obj = await this.intent.underlyingClient.getSafeAccountData<
        StoredTokenData | DeletedTokenData
      >(key);
      if (!obj) {
        obj = await this.intent.underlyingClient.getSafeAccountData<
          StoredTokenData | DeletedTokenData
        >(tokenKey(type, userId, true, instanceUrl));
      }
      if (!obj || "deleted" in obj) {
        return null;
      }
      ...
      if (keyId !== this.keyId) {
        throw new Error(
          `Stored data was encrypted with a different key to the one currently configured`,
        );
      }

      const encryptedParts =
        typeof obj.encrypted === "string" ? [obj.encrypted] : obj.encrypted;
  ...
  public async storeGenericToken(
    namespace: string,
    key: string,
    token: string,
  ) {
  ...
  public async getGitHubToken(userId: string) {
    ...
    if (senderToken.expires_in && senderToken.expires_in < date) {
      log.info(
        `GitHub access token for ${userId} has expired ${senderToken.expires_in} < ${date}, attempting refresh`,
      );
      if (!this.config.github?.oauth) {
        throw new TokenError(
          TokenErrorCode.EXPIRED,
          "GitHub oauth not configured, cannot refresh token",
        );
      }
      if (
        senderToken.refresh_token &&
        senderToken.refresh_token_expires_in &&
        senderToken?.refresh_token_expires_in > date
      ) {
        // Needs a refresh.
        ...
        senderToken = {
          ...
          expires_in:
            ...
            parseInt(refreshResult.expires_in) * 1000 + date,
          token_type: refreshResult.token_type,
          refresh_token: refreshResult.refresh_token,
          refresh_token_expires_in:
            refreshResult.refresh_token_expires_in &&
            parseInt(refreshResult.refresh_token_expires_in) * 1000 + date,
        } as GitHubOAuthToken;
        await this.storeUserToken(
          "github",
          userId,
          JSON.stringify(senderToken),
        );
      } else {
        log.error(
          `GitHub access token for ${userId} has expired, and the refresh token is stale or not given`,
        );
        throw new TokenError(
          TokenErrorCode.EXPIRED,
          `GitHub access token for ${userId} has expired, and the refresh token is stale or not given`,
  ...
  public async getOctokitForUser(userId: string) {
    ...
    return res
      ? GithubInstance.createUserOctokit(res, this.config.github.baseUrl)
      : null;
  }
  ...
  public async getOpenProjectForUser(userId: string) {
    ...
    return res
      ? new OpenProjectAPIClient(
          this.config.openProject.baseURL,
          res,
          this.openProjectOAuth,
          (newToken) => this.storeOpenProjectToken(userId, newToken),
        )
      : null;
  }
