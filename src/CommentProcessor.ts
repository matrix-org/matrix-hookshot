...
export class CommentProcessor {
  ...
  public hasCommentBeenProcessed(
    org: string,
    repo: string,
    issue: string,
    id: number,
  ) {
    return this.processedComments.has(
      `${org}/${repo}#${issue}~${id}`.toLowerCase(),
    );
  }

  public markCommentAsProcessed(
    org: string,
    repo: string,
    issue: string,
    id: number,
  ...
  public async getCommentBodyForEvent(
  ...
  public async getEventBodyForGitLabNote(
    comment: IGitLabWebhookNoteEvent,
  ...
  private replaceMentions(body: string): string {
    return body.replace(
      REGEX_MENTION,
      (match: string, part1: string, githubId: string) => {
