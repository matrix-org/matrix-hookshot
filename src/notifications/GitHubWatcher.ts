  ...
  implements NotificationWatcherTask
{
  ...
  public static checkGitHubStatus() {
    ...
    log.warn(
      `API Failure limit reached, holding off new requests for ${GH_API_RETRY_IN / 1000}s`,
    );
  ...
  private handleGitHubFailure(ex: RequestError) {
    ...
    if (ex.status === 401 || ex.status === 404) {
      log.warn(
        `Got status ${ex.status} when handing user stream: ${ex.message}`,
      );
      this.failureCount++;
  ...
  private async getNotifications() {
    if (
      ...
      log.info(
        `Not getting notifications for ${this.userId}, API is still down.`,
      );
      return;
    }
    ...
    try {
      ...
      GitHubWatcher.apiFailureCount = Math.max(
        0,
        GitHubWatcher.apiFailureCount - 2,
      );
    } catch (ex) {
      ...
      try {
        ...
        if (rawEvent.subject.latest_comment_url) {
          const res = await this.octoKit.request(
            rawEvent.subject.latest_comment_url,
          );
        ...
        if (rawEvent.reason === "review_requested") {
          ...
          ev.subject.requested_reviewers = (
