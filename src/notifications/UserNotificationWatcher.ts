...
export class UserNotificationWatcher {
  ...
  private static constructMapKey(
    userId: string,
    type: "github" | "gitlab",
    instanceUrl?: string,
  ) {
  ...
  public start() {
    ...
    this.queue.on(
      "notifications.user.enable",
      (msg: MessageQueueMessage<NotificationsEnableEvent>) => {
        this.addUser(msg.data);
      },
    );
    this.queue.on(
      "notifications.user.disable",
      (msg: MessageQueueMessage<NotificationsDisableEvent>) => {
  ...
  public removeUser(
    ...
    const key = UserNotificationWatcher.constructMapKey(
      userId,
      type,
      instanceUrl,
    );
    ...
    Metrics.notificationsWatchers.set(
      { service: type },
      this.userIntervals.size,
    );
  }
...
Check your token is still valid, and then turn notifications back on.`,
        "m.notice",
      );
    }
  }
  ...
  public addUser(data: NotificationsEnableEvent) {
    if (
      !this.config.checkPermission(
        data.userId,
        data.type,
        BridgePermissionLevel.notifications,
      )
    ) {
    ...
    if (data.type === "github") {
      ...
      task = new GitHubWatcher(
        data.token,
        this.config.github.baseUrl,
        data.userId,
        data.roomId,
        since,
        data.filterParticipating,
      );
    } else if (data.type === "gitlab" && data.instanceUrl) {
      task = new GitLabWatcher(
        data.token,
        data.instanceUrl,
        data.userId,
        data.roomId,
