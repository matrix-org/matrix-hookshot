...
export class GitLabGrantChecker extends GrantChecker<GitLabGrantConnectionId> {
  ...
  protected async checkFallback(
    roomId: string,
    connectionId: GitLabGrantConnectionId,
    sender?: string,
  ) {
    if (!sender) {
    ...
    try {
      await GitLabRepoConnection.assertUserHasAccessToProject(
      ...
      log.info(
        `${sender} does not have access to ${connectionId.instance}/${connectionId.path}`,
        ex,
      );
      return false;
    }
