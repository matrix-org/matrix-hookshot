...
const higherPriority: (a: BotUser, b: BotUser) => number = (a, b) =>
...
export default class BotUsersManager {
  ...
  private async ensureProfile(botUser: BotUser): Promise<void> {
    ...
    const canSetDisplayname =
      capabilites["m.set_displayname"]?.enabled !== false;
    ...
    try {
      profile = await botUser.intent.underlyingClient.getUserProfile(
        botUser.userId,
      );
    } catch (e) {
    ...
    if (
      ...
      try {
        await botUser.intent.underlyingClient.setDisplayName(
          botUser.displayname,
        );
        log.info(
          `Updated displayname for "${botUser.userId}" to ${botUser.displayname}`,
        );
      } catch (e) {
    ...
    if (profile.avatar_url) {
      try {
        const res = await botUser.intent.underlyingClient.downloadContent(
          profile.avatar_url,
        );
        ...
        log.error(
          `Failed to get current avatar image for ${botUser.userId}:`,
          e,
        );
      }
    }
    ...
    // Update the avatar
    try {
      const uploadedAvatarMxcUrl =
        await botUser.intent.underlyingClient.uploadContent(
          avatarImage.image,
          avatarImage.contentType,
        );
      await botUser.intent.underlyingClient.setAvatarUrl(uploadedAvatarMxcUrl);
      log.info(
        `Updated avatar for ${botUser.userId} to ${uploadedAvatarMxcUrl}`,
      );
    } catch (e) {
      log.error(`Failed to set avatar for ${botUser.userId}:`, e);
    }
  ...
  private async getJoinedRooms(): Promise<void> {
    ...
    for (const botUser of this.botUsers) {
      const joinedRooms =
        await botUser.intent.underlyingClient.getJoinedRooms();
  ...
  getBotUsersInRoom(roomId: string): BotUser[] {
    return Array.from(this._botsInRooms.get(roomId) || new Set<BotUser>()).sort(
      higherPriority,
    );
  }
