...
describe("GitLab - Pipeline Event", () => {
    ...
    const waitForMessages = (
        ...
        return new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                cleanup();
                reject(new Error(`Timeout: Expected ${expectedCount} messages, got ${receivedMessages.length}`));
            }, timeoutMs);

            const messageHandler = (eventRoomId: string, event: any) => {
                if (eventRoomId === roomId && event.sender === botMxid && event.content?.msgtype === "m.notice") {
    ...
    test("should handle GitLab pipeline success event with both messages", async () => {
        ...
        const messagesPromise = waitForMessages(user, testRoomId, testEnv.botMxid, 2);
