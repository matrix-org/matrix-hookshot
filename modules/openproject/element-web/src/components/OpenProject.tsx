  ...
  if (changes.assignee !== undefined) {
      ...
      <WidgetChangedText>
        ...
        <details
          dangerouslySetInnerHTML={{
            __html: pkg.description.html ?? pkg.description.plain,
          }}
        />
      </WidgetChangedText>
    );
    ...
    innerContent = (
      <WidgetChangedText>
        ...
        <strong>{pkg.priority?.name}</strong>
      </WidgetChangedText>
    );
    ...
    innerContent = (
      <WidgetChangedText>
        ...
        <strong>{pkg.responsible?.name ?? "Nobody"}</strong>
      </WidgetChangedText>
    );
    ...
    innerContent = (
      <WidgetChangedText>
        Status changed from{" "}
        <span>
          <WidgetWorkPackageStatus
            style={{ background: changes.status.color }}
          />
          {changes.status.name}
        </span>{" "}
        to{" "}
...
export function OpenProjectEventWidget({ data }: { data: OpenProjectContent }) {
  const pkg = data["org.matrix.matrix-hookshot.openproject.work_package"];
  if (!pkg) {
    return null;
  }
      ...
      <WidgetWrapper>
        ...
        <Root>
          ...
          <WidgetRow>
            <span>
              <WidgetWorkPackageStatus
                style={{ background: pkg.status.color }}
              />
              {pkg.status.name}
            </span>
            <span>{pkg.type.name}</span>
            {pkg.assignee?.name ? (
              <span>Assigned to {pkg.assignee.name}</span>
            ) : null}
            <span>
