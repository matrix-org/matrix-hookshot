...
class MockConnectionManager extends EventEmitter {
  constructor(public connections: IConnection[]) {
    super();
  }
...
class MockMessageQueue extends EventEmitter implements MessageQueue {
  ...
  async push(
    data: MessageQueueMessage<unknown>,
    single?: boolean,
  ): Promise<void> {
...
async function constructFeedReader(
  feedResponse: () => { headers: Record<string, string>; data: string },
) {
  ...
  const cm = new MockConnectionManager([
    { feedUrl } as unknown as IConnection,
  ]) as unknown as ConnectionManager;
  const mq = new MockMessageQueue();
  const events: MessageQueueMessage<FeedEntry>[] = [];
  mq.on("pushed", (data) => {
    if (data.eventName === "feed.entry") {
      events.push(data);
    }
  });
