...
describe("FormatUtilTest", () => {
  it("should correctly formats a repo room name", () => {
    expect(FormatUtil.formatRepoRoomName(SIMPLE_REPO)).toBe(
      "evilcorp/lab: A simple description",
    );
  });

  it("should correctly formats a issue room name", () => {
    expect(FormatUtil.formatIssueRoomName(SIMPLE_ISSUE, SIMPLE_REPO)).toBe(
      "evilcorp/lab#123: A simple title",
    );
  });

  it("should correctly generate a partial body for a Github repo", () => {
    expect(FormatUtil.getPartialBodyForGithubRepo(SIMPLE_REPO)).toEqual({
  ...
  it("should correctly generate a partial body for a Github issue", () => {
    expect(
      FormatUtil.getPartialBodyForGithubIssue(SIMPLE_REPO, SIMPLE_ISSUE),
    ).toEqual({
  ...
  it("should correctly formats a room topic", () => {
    expect(FormatUtil.formatRoomTopic(SIMPLE_ISSUE)).toBe(
      "Status: open | https://github.com/evilcorp/lab/issues/123",
    );
  });

  it("should correctly format one simple label", () => {
    expect(FormatUtil.formatLabels([{ name: "foo" }])).toEqual({
      plain: "foo",
      html: "<span>foo</span>",
    });
  });
  ...
  it("should correctly format many detailed labels", () => {
    expect(
      ...
      plain: "foo, bar",
      html:
        '<span data-mx-bg-color="#FFFFFF" data-mx-color="#000000" title="My label">foo</span> ' +
  ...
  it("should hash an ID", () => {
    expect(FormatUtil.hashId("foobar")).toBe(
