...
describe("TokenEncryption", () => {
  ...
  async function legacyEncryptFunction(token: string) {
    ...
    while (tokenSource && tokenSource.length > 0) {
      ...
      tokenParts.push(
        publicEncrypt(await keyPromise, Buffer.from(part)).toString("base64"),
      );
    }
    return tokenParts;
  }
      ...
      generateKeyPair(
        ...
        (err, _, privateKey) => {
          if (err) {
            ...
            resolve(Buffer.from(privateKey));
          }
        },
      ),
    );
    keyPromisePKCS1 = new Promise<Buffer>((resolve, reject) =>
      generateKeyPair(
        "rsa",
        {
          // Deliberately shorter length to speed up test
  ...
  it("should be able to decrypt from many parts into string", async () => {
    const plaintext =
      "This is a very long string that needs to be encoded into multiple parts in order for us to store it properly. This " +
