...
const V2TFFunction =
  "result = {plain: `The answer to '${data.question}' is ${data.answer}`, version: 'v2'}";
const V2TFFunctionWithReturn =
  "result = {plain: `The answer to '${data.question}' is ${data.answer}`, version: 'v2'}; return;";

async function testSimpleWebhook(
  connection: GenericHookConnection,
  mq: LocalMQ,
  testValue: string,
) {
  ...
  expect(await messagePromise).to.deep.equal({
    ...
    content: {
      ...
      formatted_body:
        '<p>Received webhook data:</p><p><pre><code class=\\"language-json\\">{\n  "simple": "' +
        testValue +
        '"\n}</code></pre></p>',
      msgtype: "m.notice",
...
describe("GenericHookConnection", () => {
  ...
  it("will handle a hook event containing markdown", async () => {
    ...
    expect(await messagePromise).to.deep.equal({
      ...
      content: {
        ...
        format: "org.matrix.custom.html",
        formatted_body:
          "<p><strong>bold-message</strong> <em>italic-message</em></p>",
        msgtype: "m.notice",
        "uk.half-shot.hookshot.webhook_data": webhookData,
      },
      type: "m.room.message",
  ...
  it("will handle a hook event containing a username", async () => {
    ...
    expect(await messagePromise).to.deep.equal({
      ...
      content: {
        ...
        format: "org.matrix.custom.html",
        formatted_body:
          '<strong>Bobs-integration</strong>: <p>Received webhook data:</p><p><pre><code class=\\"language-json\\">{\n  "username": "Bobs-integration",\n  "type": 42\n}</code></pre></p>',
        msgtype: "m.notice",
        "uk.half-shot.hookshot.webhook_data": webhookData,
      },
      type: "m.room.message",
  ...
  it("will handle a hook event with a v1 transformation function", async () => {
    ...
    const [connection, mq] = createGenericHook(
      { name: "test", transformationFunction: V1TFFunction },
      {
    ...
    expect(await messagePromise).to.deep.equal({
      ...
      content: {
        ...
        format: "org.matrix.custom.html",
        formatted_body:
          "<p>Received webhook: The answer to 'What is the meaning of life?' is 42</p>",
        msgtype: "m.notice",
        "uk.half-shot.hookshot.webhook_data": webhookData,
      },
      type: "m.room.message",
  ...
  it("will handle a hook event with a v2 transformation function", async () => {
    ...
    expect(await messagePromise).to.deep.equal({
      ...
      content: {
        ...
        format: "org.matrix.custom.html",
        formatted_body:
          "<p>The answer to 'What is the meaning of life?' is 42</p>",
        msgtype: "m.notice",
        "uk.half-shot.hookshot.webhook_data": webhookData,
      },
      type: "m.room.message",
  ...
  it("will handle a hook event with mentions", async () => {
    ...
    const [connection, mq] = createGenericHook(
      {
        name: "test",
        transformationFunction: `result = {plain: "Test test", mentions: { room: true, user_ids: ["@foo:bar"], ignored_data: true}, version: 'v2'}`,
      },
      {
        allowJsTransformationFunctions: true,
      },
  ...
  it("will handle a hook event with a top-level return", async () => {
    ...
    const [connection, mq] = createGenericHook(
      { name: "test", transformationFunction: V2TFFunctionWithReturn },
      {
        allowJsTransformationFunctions: true,
      },
    );
  ...
  it("will fail to handle a webhook with an invalid script", async () => {
    ...
    expect(await messagePromise).to.deep.equal({
      ...
      content: {
        ...
        format: "org.matrix.custom.html",
        formatted_body:
          "<p>Webhook received but failed to process via transformation function</p>",
        msgtype: "m.notice",
        "uk.half-shot.hookshot.webhook_data": webhookData,
      },
      type: "m.room.message",
  ...
  it("should handle simple hook events with user Id prefix", async () => {
    const [connection, mq] = createGenericHook(undefined, {
  ...
  it("should invite a configured puppet to the room if it's unable to join", async () => {
    ...
    const [connection, mq, as, botIntent] = createGenericHook(undefined, {
      userIdPrefix: "_webhooks_",
    });
    ...
    botIntent.underlyingClient.inviteUser = async (
      userId: string,
      roomId: string,
    ) => {
  ...
  it("should fail a message if a bot cannot join a room", async () => {
    const senderUserId = "@_webhooks_some-name:example.test";
    const [connection, mq, as] = createGenericHook(undefined, {
      userIdPrefix: "_webhooks_",
    });
    const intent = as.getIntentForUserId(senderUserId);
  ...
  it("should fail to create a hook with a too short expiry time", async () => {
    const as = AppserviceMock.create();
    try {
      await GenericHookConnection.provisionConnection(
        ROOM_ID,
        "@some:user",
        {
          name: "foo",
        ...
        },
        {
          ...
          intent: as.botIntent,
          config: {
            generic: new BridgeConfigGenericWebhooks(ConfigDefaults),
    ...
    try {
      await GenericHookConnection.provisionConnection(
        ...
        },
        {
          ...
          intent: as.botIntent,
          config: {
            generic: new BridgeConfigGenericWebhooks({
              ...ConfigDefaults,
              maxExpiryTime: "1d",
            }),
          } as unknown as BridgeConfig,
