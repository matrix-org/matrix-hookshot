...
async function doesAssert(
  checker: GrantChecker<string>,
  roomId: string,
  connectionId: string,
  sender?: string,
) {
...
describe("GrantChecker", () => {
  describe("base grant system", () => {
    ...
    it("will not conflict with another connection id", async () => {
      await check.grantConnection(ROOM_ID, CONNECTION_ID);
      await doesAssert(check, ROOM_ID, CONNECTION_ID + "2");
    });

    it("will not conflict with another room", async () => {
      await check.grantConnection(ROOM_ID, CONNECTION_ID);
      await doesAssert(check, ROOM_ID + "2", CONNECTION_ID);
    });

    it("will not conflict with another grant service", async () => {
      const anotherchecker = new TestGrantChecker(
        intent,
        GRANT_SERVICE + "-two",
      );
      await check.grantConnection(ROOM_ID, CONNECTION_ID);

      await doesAssert(anotherchecker, ROOM_ID, CONNECTION_ID);
    });
  });
  ...
  describe("config fallback", () => {
    ...
    it("will deny a missing grant if the sender is not in the appservice whitelist", async () => {
      await doesAssert(check, ROOM_ID, CONNECTION_ID, ALICE_USERID);
    });
